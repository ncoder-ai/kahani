...
function LoginForm() {
  ...
  // Default to "Keep me signed in" so the backend issues a refresh token.
  // Without one, the user gets a hard 2-hour session that can't be extended,
  // so the warning banner fires while they're actively using the app.
  ...
  // Clear stale tokens and initialize — runs only on mount to avoid
  // re-firing during login (useSearchParams() inside Suspense can return
  // a new reference on re-render, which would re-trigger a [searchParams] effect)
  useEffect(() => {
  ...
  const checkSSOLogin = async () => {
    ...
    try {
      ...
      if (data.access_token && data.user) {
        ...
        // Client-side nav keeps in-memory Zustand state; a hard reload
        // (window.location.href) on iOS WKWebView races with the persist
        // write and leaves the next page reading stale/empty auth.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Use router.replace for client-side nav. A hard reload via
      // window.location.href tears down the iOS WKWebView and can drop
      // in-flight Capacitor plugin calls (e.g. nativePinLock.hasPin()
      // from PinLockGate), leaving the gate stuck on its loading
      // spinner. Client-side nav keeps the bridge alive and lets the
      // gate complete its keychain check.
      ...
      try {
        ...
        if (lastStoryResponse.auto_open_last_story && lastStoryResponse.last_accessed_story_id) {
          ...
          if (storyMode === 'roleplay') {
            router.replace(`/roleplay/${lastStoryResponse.last_accessed_story_id}`);
          } else {
            router.replace(`/story/${lastStoryResponse.last_accessed_story_id}`);
          }
        } else {
          router.replace('/dashboard');
        }
  ...
  return (
    <div className="min-h-screen theme-bg-primary relative flex flex-col justify-center py-12 sm:px-6 lg:px-8 overflow-hidden">
      {/* Brand background — full-bleed image with dark overlay so the
          form on top stays legible. Same treatment as the landing page. */}
      {/* Background is a 2:1 landscape image. On portrait phones bg-cover
          would crop both sides (losing the 'Make Your Own Saga' wordmark
          that spans the full top edge). Use bg-contain anchored at the
          top so the wordmark + book artwork show as a hero strip, then
          fall back to bg-cover/center on desktop where the aspect ratio
          allows the whole image to fill cleanly. */}
      <div
        className="absolute inset-0 bg-contain md:bg-cover bg-top md:bg-center bg-no-repeat"
        style={{ backgroundImage: 'url(/kahani-background.jpg)' }}
      />
      {/* Heavier overlay at the bottom so the form is readable; lighter
          at the top so the "Make Your Own Saga" wordmark in the bg
          still reads. */}
      ...
      <div className="relative z-10 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white/10 backdrop-blur-md border border-white/20 py-8 px-6 shadow-2xl rounded-2xl">
          ...
          {/* Divider — flex layout instead of overlaying the line under
              the chip, because the chip's bg-white/10 against the card's
              bg-white/10 produced the same color, so the line showed
              through the text. */}
          <div className="mt-8">
            <div className="flex items-center gap-3">
              <div className="flex-1 border-t border-white/20" />
              <span className="text-sm text-white/70">New to Make My Saga?</span>
              <div className="flex-1 border-t border-white/20" />
            </div>
