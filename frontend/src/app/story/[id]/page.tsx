...
// TTSSettingsModal removed — the same UI lives inside SettingsModal's
// Voice tab. Story-page's "Text-to-Speech" button below opens
// SettingsModal with initialTab="voice".
...
function StoryPageContent({ storyId }: { storyId: number }) {
  ...
  // Per-scene spotlight (ephemeral): which character the next scene should
  // center on. '' = no preference (current default behavior). Reset after
  // each generation alongside customPrompt.
  ...
  // Mount-time recovery — picks up an orphaned in-flight generation that
  // started before the user navigated away. The visibility-change recovery
  // handler covers tab-backgrounded cases; this covers "user closed the
  // tab" / "user navigated to dashboard and came back."
  const hasCheckedRecoveryRef = useRef(false);
  useEffect(() => {
    if (!story || hasCheckedRecoveryRef.current) return;
    ...
    (async () => {
      try {
        ...
        if (result.status === 'generating') {
          console.log('[RECOVERY] Mount-time: in-progress generation found, resuming');
          setStreamingContent(result.content_so_far || '');
          setStreamingSceneNumber((story.scenes?.length || 0) + 1);
          setIsStreaming(true);
          // The existing handleRecovery polling loop will take over once
          // isStreaming is set; trigger it after state flushes.
          setTimeout(() => {
            apiClient.recoverGeneration(storyId).then(r => {
        ...
        if (result.status === 'completed' && result.scene_id && result.content) {
          console.log('[RECOVERY] Mount-time: completed scene available, picking up');
        ...
        if (result.status === 'error') {
          console.log('[RECOVERY] Mount-time: background generation had errored:', result.error);
          // Don't surface as a UI error — the user already navigated away from
          // this generation; just clear server state.
        }
      } catch (err) {
        console.warn('[RECOVERY] Mount-time check failed:', err);
      }
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [story, storyId]); // intentionally not depending on handleRecovery — would re-fire on every isStreaming change
  ...
  */
    ...
    try {
      ...
      await apiClient.generateSceneStreaming(
        ...
        async (sceneId: number, variantId: number, choices: any[], autoPlay?: { enabled: boolean; session_id: string; scene_id: number }, multiGen?: { isMultiGeneration: boolean; totalVariants: number; variants: any[] }, chapterId?: number | null) => {
          ...
          setCustomPrompt('');
          setFocusCharacter(''); // Spotlight is per-scene — reset after generation
                ...
                const updatedScenes = story.scenes.map(s => {
                  if (s.id === sceneId) {
                    return {
                      ...
                      choices: firstVariant.choices || s.choices
                    };
                  }
                  return s;
                });
              ...
              const updatedScenes = story.scenes.map(s => {
                if (s.id === sceneId) {
                  ...
                  return {
                    ...
                    choices: response.variant.choices || s.choices
                  };
                }
                return s;
              });
  ...
  // Real cast for the per-scene focus picker. `storyCharacters` state is only
  // populated by manual quick-add, so it's empty during normal play — the
  // authoritative chapter cast lives on activeChapter.characters (loaded from
  // GET active-chapter). role is string|null → coerce null to undefined.
  const focusCharacterOptions: Array<{ name: string; role?: string }> =
    (activeChapter?.characters ?? []).map((c: { name: string; role?: string | null }) => ({
      name: c.name,
  ...
  return (
    <div className="min-h-screen theme-bg-primary text-white">
                    ...
                    return (
                      ...
                      >
                        ...
                        <SceneVariantDisplay
                          ...
                          storyCharacters={focusCharacterOptions}
                          focusCharacter={focusCharacter}
                          onFocusCharacterChange={setFocusCharacter}
            ...
            {directorMode && story?.scenes && story.scenes.length > 0 && (
              <div className="mb-8 space-y-4">
                <div className="bg-gray-700 rounded-xl border border-gray-600 p-4">
                  ...
                  />
                  {focusCharacterOptions.length > 0 && (
                    <div className="mt-3">
                      <label className="block text-xs font-medium text-gray-400 mb-1">
                        Focus this scene on (optional)
                      </label>
                      <select
                        value={focusCharacter}
                        onChange={(e) => setFocusCharacter(e.target.value)}
                        disabled={isGenerating || isStreaming}
                        className="w-full bg-gray-800 border border-gray-600 rounded-lg p-2 text-sm text-gray-200 focus:outline-none focus:border-pink-500 disabled:opacity-50"
                      >
                        <option value="">No focus — let the AI decide</option>
                        {focusCharacterOptions.map((character) => (
                      ...
                      <span className="text-xs text-gray-500 mt-1 block">
                        Gives this character the most narrative weight. POV is unchanged.
                      </span>
                    </div>
                  )}
      ...
      {/* Settings Modal opened on the Voice tab — replaces the old
          standalone TTSSettingsModal. storyId is read by VoiceSettingsTab
          via useParams() so the per-story Character voices section
      ...
      {/* Floating Cancel Button — always reachable during generation.
          Without this, the only stop control was buried in SceneVariantDisplay's
          floating menu, leaving mobile users with no escape from stalled streams. */}
      {(isStreaming || isStreamingContinuation || isRegenerating || isGenerating) && !isInDeleteMode && (
        <div
        ...
        >
          <button
            ...
            className="flex items-center gap-2 px-5 py-3 bg-red-600 hover:bg-red-700 active:bg-red-800 text-white rounded-full shadow-2xl font-medium transition-all border-2 border-red-500/70 backdrop-blur-sm"
            title="Cancel generation"
          >
            <StopIcon className="w-5 h-5" />
            <span className="text-sm">Cancel generation</span>
          </button>
        </div>
      )}
