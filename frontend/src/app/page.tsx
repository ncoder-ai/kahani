...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen theme-bg-primary">
      {/* Header — Sign In / Get Started, desktop only. Sits on the dark
          bar above the banner so it never overlaps the wordmark. On
          mobile the hero section below has the same CTAs. */}
      ...
      {/* Brand banner — a real <img> so it keeps its natural 2:1 aspect
          ratio: never stretched, never blown up. h-auto lets it size to
          its width on phones (full wordmark + book visible); on tall
          desktop viewports max-h-[60vh] caps it and object-top trims
          only the lower decorative edge, keeping the wordmark + book.
          The gradient feathers the bottom into the dark page. */}
      <div className="relative">
        <img
          src="/kahani-background.jpg"
          alt="Make Your Own Saga"
          className="w-full h-auto max-h-[60vh] object-cover object-top"
        />
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-24 bg-gradient-to-b from-transparent to-[#0a0a0f]" />
      </div>

      {/* Hero content flows naturally below the banner — no vw padding
          hacks needed since the image is a real flow element now. */}
      <main className="max-w-7xl mx-auto px-6 pt-10 pb-24">
        <div className="text-center mb-20">
          ...
          <p className="text-xl text-white/80 mb-12 max-w-3xl mx-auto">
            Make My Saga is an interactive storytelling platform that combines your creativity with AI assistance
        ...
        <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-12 text-center">
          ...
          <p className="text-white/80 text-lg mb-8">
            Join thousands of creators who are bringing their imagination to life with Make My Saga
          </p>
          <Link
            href="/register"
            className="theme-btn-primary px-8 py-4 rounded-2xl font-semibold text-lg transform hover:scale-105 transition-all duration-200 shadow-lg"
      ...
      <footer className="relative z-10 bg-white/5 backdrop-blur-md border-t border-white/20 mt-20">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="text-center text-white/60">
            <p>&copy; {new Date().getFullYear()} Make My Saga. Unleashing creativity through interactive storytelling.</p>
          </div>
        </div>
      </footer>
    </div>
