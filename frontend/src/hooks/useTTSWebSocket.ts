...
interface WebSocketMessage {
  type: 'chunk_ready' | 'stream_start' | 'frame' | 'stream_end' | 'progress' | 'complete' | 'error';
  // chunk_ready (legacy complete-chunk providers)
  chunk_number?: number;
  total_chunks?: number;
  audio_base64?: string;
  text_preview?: string;
  size_bytes?: number;
  duration?: number;
  // stream_start / frame / stream_end (PCM frame streaming)
  stream_id?: string;
...
export const useTTSWebSocket = ({
  ...
  /**
   * Mark playback as started + register the playback-end callback.
   * Shared between the legacy chunk_ready path and the PCM frame path
   * so both kinds of producers go through identical bookkeeping.
   */
  ...
  }, [onPlaybackStart, handlePlaybackEnd]);

  /**
   * Translate an AudioContext exception into a user-friendly error
   * (specifically, surface the "tap to enable audio" hint when iOS
   * Safari has not been unlocked yet).
   */
  const reportAudioError = useCallback((err: unknown, what: string) => {
    const errorMsg = err instanceof Error ? err.message : `Failed to ${what}`;
  ...
  /**
   * PCM streaming handlers (Qwen3 + future PCM-streaming providers)
   */
  const handleStreamStart = useCallback((message: WebSocketMessage) => {
    if (!message.stream_id || !message.sample_rate) {
      console.warn('[TTS WS] stream_start missing fields', message);
      return;
    }
    audioContextManager.beginPcmStream(message.stream_id, {
      sampleRate: message.sample_rate,
      channels: message.channels ?? 1,
      bitsPerSample: message.bits_per_sample ?? 16,
    });
    if (message.total_chunks) setTotalChunks(message.total_chunks);
  }, []);

  const handleStreamFrame = useCallback(async (message: WebSocketMessage) => {
    if (!message.stream_id || !message.pcm_base64) return;
    try {
      ...
      // First frame of the first stream is the "playback started" moment.
      if (!isPlayingRef.current) markPlaybackStarted();
    } catch (err) {
      console.error('[TTS WS] PCM frame failed:', err);
      reportAudioError(err, 'play PCM frame');
    }
  }, [markPlaybackStarted, reportAudioError]);

  const handleStreamEnd = useCallback((message: WebSocketMessage) => {
  ...
  }, [queueAudioChunk, onProgress, onError, handlePlaybackEnd, handleStreamStart, handleStreamFrame, handleStreamEnd]);
