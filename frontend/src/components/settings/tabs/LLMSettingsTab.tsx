...
export default function LLMSettingsTab({
  ...
  // Resolve the display label for a configured provider. Priority:
  //   1. user's custom label  (set via the rename / Add-Provider name field)
  //   2. catalog label looked up by the entry's api_type
  //   3. raw slug
  // The list and the role dropdowns must agree on this — otherwise a user-named
  // entry like "ikllama-main" (label "Local llama-server") shows up under two
  // different names in the same modal.
  const resolveProviderLabel = (pid: string): string => {
    const creds = configuredProviders[pid];
    if (creds?.label) return creds.label;
    const apiType = creds?.api_type || pid;
    const info =
      providers.find((p) => p.id === apiType) ||
      embeddingProviders.find((p) => p.id === apiType);
    return info?.label || pid;
  };

  // Add a new provider to the registry
  // Slugify a free-form name into a safe configured_providers dict key.
  const slugifyName = (s: string): string =>
    s.trim().toLowerCase().replace(/[^a-z0-9-]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 80);

  const addProvider = (providerId: string, apiKey: string, apiUrl: string, name?: string): boolean => {
    // Name is REQUIRED so every provider has a stable, user-meaningful display
    // label that the list and the role dropdowns can show consistently.
    if (!name || !name.trim()) {
      showMessage('Name is required. Give this provider a name you\'ll recognize (e.g. "OpenRouter prod", "local llama-server").', 'error');
      return false;
    }
    const slug = slugifyName(name);
    const label = name.trim();
    if (!slug) {
      showMessage('Name must contain at least one letter or number', 'error');
      return false;
    }
    if (configuredProviders[slug]) {
      showMessage(`A provider named "${slug}" already exists. Pick a different name.`, 'error');
      return false;
    }
    setConfiguredProviders({
      ...configuredProviders,
  ...
  // Update an existing provider — credentials AND optional rename. Returns
  // false (and shows an error) if a rename collides with another slug.
  // On rename, any active-role pointers (currentEngine, currentExtractionEngine,
  // embedding provider) that referenced the old slug are updated to the new
  // slug so the user doesn't silently lose their active selection.
  const updateProvider = (
    ...
    const existing = configuredProviders[oldId];
    ...
    if (newName !== undefined) {
      ...
      if (trimmed) {
        const proposedSlug = slugifyName(trimmed);
        if (!proposedSlug) {
          showMessage('Name must contain at least one letter or number', 'error');
          return false;
        }
        if (proposedSlug !== oldId) {
          if (configuredProviders[proposedSlug]) {
            showMessage(`A provider named "${proposedSlug}" already exists. Pick a different name.`, 'error');
            return false;
          }
          newId = proposedSlug;
        }
        ...
        // User cleared the name → drop the label, keep the dict key as-is
        nextLabel = undefined;
      }
    }

    const next: ConfiguredProviders = { ...configuredProviders };
    if (newId !== oldId) {
      delete next[oldId];
    }
    next[newId] = {
    ...
    if (newId !== oldId && !nextLabel) {
      // Field was explicitly cleared
      delete next[newId].label;
    }
    setConfiguredProviders(next);

    // If this entry was selected as an active role, update those pointers
    // so the user's selection survives the rename.
    if (newId !== oldId) {
      if (currentEngine === oldId) setCurrentEngine(newId);
      if (currentExtractionEngine === oldId) setCurrentExtractionEngine(newId);
      if (embeddingSettings.provider === oldId) {
        setEmbeddingSettings({ ...embeddingSettings, provider: newId, provider_id: newId });
      }
    }
    return true;
  };
  ...
  const [newProviderName, setNewProviderName] = useState('');  // optional custom slug (e.g. "nishant-mac")
  const [editingProvider, setEditingProvider] = useState<string | null>(null);
  const [editProviderKey, setEditProviderKey] = useState('');
  const [editProviderUrl, setEditProviderUrl] = useState('');
  const [editProviderName, setEditProviderName] = useState('');  // optional rename
  ...
  // Show all provider types, even already-configured ones. addProvider()
  // generates a numbered slug suffix so multiple instances of the same
  // api_type can coexist (e.g. two openai-compatible local servers).
  ...
  const handleTestAndAdd = async () => {
    ...
    try {
      ...
      const result = await response.json();
      if (result.success) {
        const ok = addProvider(newProviderType, newProviderKey, newProviderUrl, newProviderName);
        if (!ok) return;  // addProvider already showed an error (slug collision)
        setAddingProvider(false);
        setNewProviderType('');
        setNewProviderKey('');
        setNewProviderUrl('');
        setNewProviderName('');
        showMessage(`Added ${newProviderName || providerInfo.label}`, 'success');
      } else {
        ...
        <div className="mb-6 p-4 bg-gray-800/50 rounded-lg border border-gray-700">
          ...
          <div className="space-y-2">
            {configuredProviderIds.map(pid => {
              const creds = configuredProviders[pid];
              // For multi-instance entries the dict key (pid) is a custom slug
              // (e.g. "nishant-mac") and the actual api_type lives inside the
              // entry. Look up provider metadata by the resolved api_type so
              // the category badge and provider label still resolve correctly.
              const apiType = creds?.api_type || pid;
              const providerInfo = providers.find(p => p.id === apiType) || embeddingProviders.find(p => p.id === apiType);
              const label = creds?.label || providerInfo?.label || pid;
                    ...
                    {pid !== 'local' && (
                      <>
                        <button
                          onClick={() => {
                            if (isEditing) {
                              const ok = updateProvider(pid, editProviderKey, editProviderUrl, editProviderName);
                              if (!ok) return;  // collision or validation error already shown
                              ...
                              setEditProviderName(creds?.label || '');  // empty if entry has no custom label
                            }
                          }}
            ...
            {editingProvider && editingProvider !== 'local' && (
              <div className="p-3 bg-gray-700/30 rounded-lg border border-gray-600 space-y-2 mt-1">
                {(() => {
                  // Resolve provider info via the entry's stored api_type so
                  // custom-named entries (slug != api_type) still surface the
                  // right cloud/local distinction.
                  const editEntry = configuredProviders[editingProvider];
                  const editApiType = editEntry?.api_type || editingProvider;
                  const providerInfo = providers.find(p => p.id === editApiType);
                  const isCloud = providerInfo?.category === 'cloud';
                  return (
                    <>
                      <div>
                        <label className="block text-xs text-gray-400 mb-1">
                          Name <span className="text-gray-500">(rename this provider — leave blank to clear the custom name)</span>
                        </label>
                        <input
                          type="text"
          ...
          {addingProvider && (
            <div className="mt-3 p-3 bg-gray-700/30 rounded-lg border border-gray-600 space-y-3">
              <div>
                <label className="block text-xs text-gray-400 mb-1">
                  Name <span className="text-red-400">*</span> <span className="text-gray-500">(required — shown in the list and dropdowns; pick something you'll recognize)</span>
                </label>
                <input
                  type="text"
                  value={newProviderName}
                  onChange={(e) => setNewProviderName(e.target.value)}
                  placeholder='e.g. "OpenRouter prod" or "local llama-server"'
                  className="w-full bg-gray-700 border border-gray-600 rounded px-2.5 py-1.5 text-sm text-white"
                />
              </div>
              <div>
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                <button
                  ...
                  disabled={!newProviderType || !newProviderName.trim()}
                  className="w-full sm:w-auto px-3 py-1.5 text-sm bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white rounded font-medium"
                >
                  Test & Add
                </button>
                <button
                  ...
                  disabled={!newProviderType || !newProviderName.trim()}
                  className="w-full sm:w-auto px-3 py-1.5 text-sm bg-gray-600 hover:bg-gray-500 disabled:opacity-50 text-white rounded font-medium"
                >
                  Add Without Test
                </button>
                <button
                  onClick={() => { setAddingProvider(false); setNewProviderType(''); setNewProviderKey(''); setNewProviderUrl(''); setNewProviderName(''); }}
                  className="w-full sm:w-auto px-3 py-1.5 text-sm text-gray-400 hover:text-white"
                >
                  Cancel
                </button>
              </div>
                ...
                <div>
                  ...
                  >
                    ...
                    {configuredProviderIds.filter(pid => pid !== 'local').map(pid => (
                      <option key={pid} value={pid}>{resolveProviderLabel(pid)}</option>
                    ))}
                  </select>
                ...
                <div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    <input
                      ...
                      className="w-full sm:flex-1 min-w-0 bg-gray-700 border border-gray-600 rounded-md px-3 py-2 text-white"
                      placeholder={isCloudProvider(currentEngine) ? 'model-name' : 'gpt-4'}
                    />
                    <button
                      onClick={fetchAvailableModels}
                      disabled={loadingModels || autoLoading || !currentEngine}
                      className="w-full sm:w-auto px-4 py-2 theme-btn-primary rounded-md font-medium disabled:opacity-50"
                    >
                ...
                <div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    <input
                      ...
                      disabled={pollingReembed}
                      className="w-full sm:flex-1 min-w-0 bg-gray-700 border border-gray-600 rounded-md px-3 py-2 text-white disabled:opacity-50"
                    ...
                    {embeddingSettings.provider !== 'local' && (
                      <button
                        ...
                        className="w-full sm:w-auto px-3 py-2 bg-gray-600 hover:bg-gray-500 disabled:bg-gray-700 disabled:cursor-not-allowed text-white rounded text-sm whitespace-nowrap"
                      >
