  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Notify open SceneImageGenerator panels to re-probe server status,
        // since they cache the online/offline check from mount time.
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('kahani:imageGenSettingsUpdated'));
        }
      } else {
            ...
            <div>
              ...
              <div className="flex gap-2">
                <select
                  ...
                  className="flex-1 min-w-0 bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-purple-500 disabled:opacity-50"
                >
