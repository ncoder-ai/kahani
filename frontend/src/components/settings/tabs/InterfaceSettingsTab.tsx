...
import { isNative, disconnect } from '@/lib/capacitor';
...
export default function InterfaceSettingsTab({
  ...
  // SSR-safe: only show mobile-app section after mount (avoids hydration mismatch)
  const [showMobileSection, setShowMobileSection] = useState(false);
  useEffect(() => { setShowMobileSection(isNative()); }, []);
    ...
    <div className="space-y-6">
      ...
      {showMobileSection && (
        <div className="border-t border-gray-700 pt-4 mt-4">
          <h3 className="text-lg font-semibold text-white mb-3">Mobile App</h3>
          <div className="space-y-3">
            <div>
              <label className="block text-sm text-gray-400 mb-1">Connected to</label>
              <div className="px-3 py-2 bg-gray-800 border border-gray-700 rounded-md text-white text-sm break-all">
            ...
            <button
              onClick={async () => {
                ...
                if (!window.confirm(
                  `Disconnect from ${origin}?\n\nYou'll need to enter your server URL again on next launch.`
                )) return;
                await disconnect();
              }}
              className="w-full sm:w-auto px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-md font-medium"
            >
