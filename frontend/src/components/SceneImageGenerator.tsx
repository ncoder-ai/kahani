...
export default function SceneImageGenerator({
  ...
  // Load server status and available models. Extracted so we can also
  // call it from a manual refresh button and from the settings-saved
  // event listener.
  ...
  // Re-probe when the user saves new image-gen settings — the URL or
  // API key may have changed and the panel's cached online/offline flag
  // would otherwise stay stuck on Offline until full reload.
  useEffect(() => {
    const handler = () => { loadServerInfo(); };
    window.addEventListener('kahani:imageGenSettingsUpdated', handler);
    return () => window.removeEventListener('kahani:imageGenSettingsUpdated', handler);
  }, [loadServerInfo]);
  ...
  return (
    <div className="mt-4 pt-4 border-t border-white/10">
      ...
      {!collapsed && <div className="flex items-center gap-2">
        ...
        {/* Server status indicator + retry button */}
        <button
          type="button"
