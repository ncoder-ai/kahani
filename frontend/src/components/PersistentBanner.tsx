...
export default function PersistentBanner() {
  ...
  // Hide explicitly on logged-out routes. The `!user` check above usually
  // handles this, but the persisted Zustand store can briefly hydrate a
  // stale user before /login's mount-effect calls logout(), causing a
  // banner flash on the login screen.
  if (
    pathname === '/login' ||
    pathname === '/register' ||
  ...
  return (
    <>
      {/* Top Banner — opaque gradient with strong shadow for visual separation.
          Dropped backdrop-blur-md: iOS Safari + WKWebView renders blurred
          fixed elements with subtle transparency that lets content show
          through. theme-banner uses solid hex colors with no alpha. */}
      <div className="fixed top-0 left-0 right-0 z-50 theme-banner border-b border-white/20 shadow-lg" suppressHydrationWarning>
        <div className="max-w-7xl mx-auto px-2 md:px-4 py-0.5 md:py-1">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center space-x-1 md:space-x-3 min-w-0 flex-1" suppressHydrationWarning>
              {/* Back button - desktop only (mobile uses swipe-back / menu) */}
              {isClient && canGoBack && (
                <button
                  onClick={handleBack}
                  className="hidden md:flex items-center text-white/80 hover:text-white hover:bg-white/10 p-1 rounded-md transition-all duration-200 flex-shrink-0"
                  title="Go back"
                >
              ...
              {/* Logo — mobile: tap to open menu (44x44 hit area).
                  Desktop: tap to go home (dedicated menu icon lives on
                  the right). Image is masked into a circle with a soft
                  radial fade so the JPEG's colored background blends
                  into the banner instead of looking like a hard-edged
                  square sticker. */}
              <button
                onClick={() => setShowUnifiedMenu(true)}
                className="md:hidden flex items-center justify-center text-white flex-shrink-0 leading-none w-11 h-11 -ml-1"
              ...
              >
                <img
                  ...
                  style={{
                    WebkitMaskImage: 'radial-gradient(circle, black 55%, transparent 92%)',
              ...
              <button
                onClick={handleHome}
                className="hidden md:flex items-center text-white flex-shrink-0 leading-none"
              >
                <img
                  src="/kahani-logo.jpg"
                  alt="Make My Saga"
              ...
              {isClient && pathname?.startsWith('/story/') && storyActions?.storyTitle ? (
                <>
                  ...
                  <span
                    className="text-white/90 text-xs md:text-sm font-medium truncate min-w-0 flex-1 leading-none"
            ...
            {/* Right side - Status dot (desktop), actions */}
            <div className="flex items-center space-x-1 md:space-x-2">
              {/* Status indicator - desktop only. The big red "Stop Generation"
                  banner already conveys generation state on mobile, and
                  extraction/idle states aren't actionable. */}
              {getStatusColor() && (
                <div
                  className="hidden md:flex items-center gap-1.5 px-2 py-1 text-[10px] text-white/80"
                  title={
              ...
              {storyActions?.showCharacterBanner && storyActions?.onDiscoverCharacters && (
                <button
                  ...
                  className="relative flex items-center justify-center w-9 h-9 md:w-auto md:h-auto md:px-2.5 md:py-1.5 rounded-md transition-all hover:bg-white/10 text-amber-400"
              ...
              {/* Menu Button — desktop only. On mobile the logo opens the menu. */}
              <button
                onClick={() => setShowUnifiedMenu(true)}
                className="hidden md:flex items-center justify-center w-7 h-7 rounded-md transition-all hover:bg-white/10 text-white/80 hover:text-white"
                aria-label="Open menu"
              >
                <MenuIcon className="w-4 h-4" />
              </button>

              {/* TTS Audio Unlock — only on mobile Safari (not the Capacitor
                  iOS app, which uses the native Swift TTS plugin and never
                  touches Web Audio). */}
              {isClient && isMobile && !isNative() && (
                <button
