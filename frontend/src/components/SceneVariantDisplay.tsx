...
interface SceneVariantDisplayProps {
  ...
  // Per-scene character spotlight (ephemeral). '' = no preference.
  storyCharacters?: Array<{ name: string; role?: string }>;
  focusCharacter?: string;
  onFocusCharacterChange?: (name: string) => void;
...
export default function SceneVariantDisplay({
  ...
  // We treat the button as "Stop" whenever this scene is the active TTS
  // session (playing OR generating) — matches SceneTTSButton, so the two
  // controls don't diverge during the generate-before-first-chunk window.
  const { playScene, stop, currentSceneId, isPlaying: isTTSPlaying, isGenerating: isTTSGenerating } = useGlobalTTS();
  const ttsActiveForScene = (sceneId: number) => currentSceneId === sceneId && (isTTSPlaying || isTTSGenerating);
  ...
  return (
    <div ref={sceneContentRef} className="scene-variant-container">
          ...
          >
            ...
            {showFloatingMenu && (
              <div className="absolute right-16 md:right-20 bottom-0 space-y-2 animate-fade-in">
                ...
                {/* Play/Stop TTS — distinct visual for the "preparing" gap
                    between click and first audio frame so the user can tell
                    "processing" from "no-op". */}
                {(() => {
                  const isActive = ttsActiveForScene(scene.id);
                  const isPreparing = isActive && !isTTSPlaying;  // generating but no audio yet
                  const label = isPreparing ? 'Preparing audio…' : isActive ? 'Stop TTS' : 'Play TTS';
                  return (
                    <button
                      onClick={() => {
                        setShowFloatingMenu(false);
      ...
      {isLastScene && (
        <div className="mt-6">
          {/* Per-scene character spotlight — applies to whichever way the
              next scene is generated (a choice tap or the custom input) */}
          {!directorMode && storyCharacters.length > 0 && (
            <div className="mb-3 flex items-center gap-2">
              ...
              <select
                ...
                onChange={(e) => onFocusCharacterChange?.(e.target.value)}
                disabled={isGenerating || isStreaming}
                className="flex-1 min-w-0 bg-gray-800 border border-gray-600 rounded-lg px-2 py-1.5 text-sm text-gray-200 focus:outline-none focus:border-pink-500 disabled:opacity-50"
                title="Give this character the most narrative weight in the next scene. POV is unchanged."
              >
