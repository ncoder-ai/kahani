...
class Story(Base):
    ...
    # Per-story TTS character → voice mapping, namespaced by provider so
    # switching from qwen3-tts → kokoro → openai-compatible preserves each
    # provider's mapping independently. Lookup key is the lowercased
    # character name; absence from the map means "use the user's default
    # voice from tts_settings". The synthesis-time lookup happens in
    # tts_service when Phase B+C lands. Shape:
    #   { "qwen3-tts": {"narrator": "Aiden", "john": "clone:alan_rickman"} }
    tts_character_voices = Column(JSON, nullable=True)
