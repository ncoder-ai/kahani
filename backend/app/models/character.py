...
class StoryCharacter(Base):
    ...
    # Story-specific narrative presence (Feature 3 — "peanut-butter" fix).
    # JSON: {"presence": "low"|"normal"|"high", "expression": "internal"|"balanced"|"spoken"}
    # null OR presence=normal+expression=balanced => no-op (byte-identical prompt).
    # presence = how much narrative space the character occupies (volume);
    # expression = whether that space is dialogue (spoken) or interiority (internal).
    narrative_presence = Column(JSON, nullable=True)
