...
class SceneVariant(Base):
    ...
    # Cached output of the TTS scene-segment extractor (Phase B+C). When
    # `use_segment_extraction` is enabled in a user's TTSSettings, this
    # column stores the per-speaker segment list so subsequent plays of
    # the same variant skip the extraction LLM call. Cleared whenever the
    # variant content changes (see invalidate_extractions_for_scene).
    # Shape: {"model": "...", "extracted_at": "...", "segments": [...]}.
    tts_segments = Column(JSON, nullable=True)
