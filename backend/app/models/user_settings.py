...
class UserSettings(Base):
    ...
    # NOTE: llm_api_type / llm_api_url / llm_api_key are LEGACY single-instance fields.
    # The new multi-instance design stores per-provider config in `configured_providers`
    # (JSON, keyed by user-given slug) and tracks the active provider via
    # `llm_provider_id`. The legacy columns are retained for backwards compat —
    # used as a fallback when llm_provider_id is None or doesn't resolve.
    ...
    llm_provider_id = Column(String(100), nullable=True)  # Active provider slug into configured_providers
    ...
    # BCP-47 locale (e.g. "en-IN", "en-US", "hi-IN"). NULL = use device/server
    # default. iOS passes this to SFSpeechRecognizer; backend Whisper takes
    # the 2-letter prefix ("en", "hi") since faster-whisper uses ISO-639-1.
    stt_language = Column(String(16), nullable=True)
    
    # Extraction Model Settings
    # NOTE: `extraction_model_url` and `extraction_model_api_key` here, like
    # `llm_api_url` / `llm_api_key` above (see header note), are LEGACY columns
    # that may be STALE relative to `configured_providers`. Production code
    # paths (to_dict, _resolve_provider_credentials, settings.py:1919 etc.)
    # call `_get_active_provider("extraction")` first and only fall back to
    # these legacy columns when no `configured_providers` entry exists.
    # Do NOT read these columns directly in new code; use
    # `_resolve_provider_credentials(extraction_provider_id)` or
    # `_get_active_provider("extraction")`. Reading them directly may give
    # the wrong URL for users with multi-provider setups.
    ...
    use_main_llm_for_scene_events = Column(Boolean, nullable=True)  # Use main LLM for per-scene event extraction (recall index)
    ...
    extraction_provider_id = Column(String(100), nullable=True)  # Active extraction provider slug into configured_providers (multi-instance)
    ...
    embedding_provider_id = Column(String(100), nullable=True)  # Active embedding provider slug into configured_providers (multi-instance)
    ...
    def _resolve_provider_credentials(self, provider_id: str) -> tuple:
        ...
        Kept for backwards compat — new code should use _get_provider_entry().
        """
        entry = self._get_provider_entry(provider_id)
        if entry:
            return (entry.get("api_key", ""), entry.get("api_url", ""))

        # Legacy fallback — check flat columns by api_type match
    ...
    def _get_provider_entry(self, provider_id: str) -> dict:
        """Return the configured_providers entry for a slug, with api_type resolved.

        For legacy entries (where the dict key IS the api_type and no `api_type`
        field is stored inside), the key itself is used as the api_type. New
        entries store an explicit `api_type` field so the slug can be anything
        the user wants.
        ...
        if not entry:
            return {}
        # Materialize a copy with api_type guaranteed (legacy default = key)
        out = dict(entry)
        out.setdefault("api_type", provider_id)
        return out

    def _get_active_provider(self, role: str) -> dict:
        """Return the active provider entry for a role: 'llm' | 'extraction' | 'embedding'.

        Resolution order:
          1. New: <role>_provider_id slug → configured_providers[slug]
          2. Legacy: <role>_api_type / embedding_provider as the slug
          3. Empty dict if nothing resolves

        The returned dict is guaranteed to have 'api_type' set.
        """
        # Step 1: explicit provider_id (new multi-instance mode)
        explicit_id = None
        if role == "llm":
            explicit_id = self.llm_provider_id
        elif role == "extraction":
            explicit_id = self.extraction_provider_id
        elif role == "embedding":
            explicit_id = self.embedding_provider_id

        if explicit_id:
            entry = self._get_provider_entry(explicit_id)
            if entry:
                entry["provider_id"] = explicit_id
                return entry

        # Step 2: legacy slug-equals-api_type lookup
        legacy_id = None
        if role == "llm":
            legacy_id = self.llm_api_type
        elif role == "extraction":
            legacy_id = self.extraction_model_api_type
        elif role == "embedding":
            legacy_id = self.embedding_provider

        if legacy_id:
            entry = self._get_provider_entry(legacy_id)
            if entry:
                entry["provider_id"] = legacy_id
                return entry
            # No entry exists but we have a legacy api_type — synthesize from flat columns
            return self._synthesize_legacy_entry(role, legacy_id)

        return {}

    def _synthesize_legacy_entry(self, role: str, api_type: str) -> dict:
        """Build a virtual provider entry from the legacy flat columns.

        Used when llm_api_type (etc.) is set but configured_providers has no
        matching entry. Lets old user_settings rows resolve cleanly through
        the new code path without requiring a data migration.
        """
        if role == "llm":
            return {
        ...
        if role == "extraction":
            return {
                ...
                "api_url": self.extraction_model_url or "",
            }
        if role == "embedding":
            return {
                "provider_id": api_type,
                ...
                "api_url": self.embedding_api_url or "",
            }
        return {}
    ...
    def to_dict(self):
        ...
        # Resolve via the multi-instance helper. Falls back transparently to
        # legacy flat columns + slug-equals-api_type if no llm_provider_id is
        # set (so existing users keep working without migration).
        llm_active = self._get_active_provider("llm")
        llm_provider_id = llm_active.get("provider_id") or llm_api_defaults.get("api_type", "openai-compatible")
        llm_provider = llm_active.get("api_type") or llm_api_defaults.get("api_type", "openai-compatible")
        llm_key = llm_active.get("api_key") or self.llm_api_key or ""
        llm_url = llm_active.get("api_url") or self.llm_api_url or ""
        # Each entry can carry per-role config (llm/extraction/embedding sub-
        # dicts). When the active entry has them, prefer those over the flat
        # legacy columns so model_name etc. travel with the provider — this
        # is what lets users park different model names on different
        # provider entries and switch active without overwriting state.
        llm_role = llm_active.get("llm") if isinstance(llm_active.get("llm"), dict) else {}

        ext_active = self._get_active_provider("extraction")
        ext_provider_id = ext_active.get("provider_id") or ext_model_defaults.get("api_type", "openai-compatible")
        ext_provider = ext_active.get("api_type") or ext_model_defaults.get("api_type", "openai-compatible")
        ext_key = ext_active.get("api_key") or self.extraction_model_api_key or ""
        ext_url = ext_active.get("api_url") or self.extraction_model_url or ext_model_defaults.get("url", "http://localhost:1234/v1")
        ext_role = ext_active.get("extraction") if isinstance(ext_active.get("extraction"), dict) else {}

        emb_active = self._get_active_provider("embedding")
        emb_provider_id = emb_active.get("provider_id") or "local"
        emb_provider = emb_active.get("api_type") or "local"
        emb_key = emb_active.get("api_key") or self.embedding_api_key or ""
        emb_url = emb_active.get("api_url") or self.embedding_api_url or ""
        emb_role = emb_active.get("embedding") if isinstance(emb_active.get("embedding"), dict) else {}
        ...
        return {
            "llm_settings": {
                ...
                "provider_id": llm_provider_id,  # Active provider slug (multi-instance support)
                # Prefer per-entry model_name (multi-instance) over the flat
                # legacy column. Without this, switching the active provider
                # leaves the old provider's model_name attached and the new
                # provider rejects the request.
                "model_name": llm_role.get("model_name") or self.llm_model_name or "",
            ...
            "stt_settings": {
                ...
                "language": self.stt_language if self.stt_language is not None else stt_defaults.get("language", "")
            },
            "extraction_model_settings": {
                ...
                "url": ext_url,
                "api_key": ext_key,
                "model_name": ext_role.get("model_name") or self.extraction_model_name or ext_model_defaults.get("model_name", "qwen2.5-3b-instruct"),
                ...
                "use_main_llm_for_scene_events": self.use_main_llm_for_scene_events if self.use_main_llm_for_scene_events is not None else False,
                ...
                "api_type": ext_provider,
                "provider_id": ext_provider_id,  # Active provider slug (multi-instance support)
            },
            ...
            "embedding_model_settings": {
                ...
                "provider_id": emb_provider_id,  # Active provider slug (multi-instance support)
                "api_url": emb_url,
                "api_key": emb_key,
                "model_name": emb_role.get("model_name") or self.embedding_model_name or settings.semantic_embedding_model,
    ...
    def populate_from_defaults(self):
        ...
        if self.stt_language is None:
            self.stt_language = stt.get("language", "")
