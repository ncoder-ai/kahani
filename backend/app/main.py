...
# Health check endpoint — kept minimal so it can be exempted from upstream
# auth (Authelia, etc.) without leaking version/uptime/server-time. Consumed
# by Docker HEALTHCHECK, start-*.sh scripts, health-check.sh, and the
# Capacitor launcher's probe (which only validates `status` and `app`).
@app.get("/health")
