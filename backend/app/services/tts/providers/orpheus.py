...
Per-utterance emotion is delivered via inline angle-bracket paralinguistic
tags (Orpheus's native scheme — see canopyai/Orpheus-TTS README). The
LLM-extracted emotion phrase is mapped to one of 8 supported tags via
the shared `emotion_to_tag` utility.
"""
...
# Per-provider tag table — order = priority. Orpheus only supports
# paralinguistic tags (no broad emotional tags like [angry] / [happy]),
# so phrases that map to those broader intents fall through to no tag.
_ORPHEUS_TAGS = {
    "laugh":   "<laugh>",
    "sigh":    "<sigh>",
    "gasp":    "<gasp>",
...
class OrpheusProvider(TTSProviderBase):
    ...
    def _input_with_tag(self, text: str, instructions: Optional[str]) -> str:
        """Prepend an Orpheus paralinguistic tag inline if the LLM emotion
        phrase maps to one of the 8 supported angle-bracket tags. Returns
        the original text if no intent matches — the dialogue plays
        without a tag, which is the correct fallback.

        Orpheus tag set (canopyai/Orpheus-TTS README):
            <laugh> <chuckle> <sigh> <cough> <sniffle>
            <groan> <yawn> <gasp>
        Notably emotion-only tags (angry, sad, etc.) aren't supported —
        Orpheus relies on the surrounding text to convey emotional tone.
        """
        from ..emotion_to_tag import select_tag
        tag = select_tag(instructions, _ORPHEUS_TAGS)
    ...
    async def synthesize(self, request: TTSRequest) -> TTSResponse:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.config.timeout) as client:
                ...
                # Build request payload — inject Orpheus paralinguistic
                # tag inline if the per-utterance emotion phrase maps to one.
                input_text = self._input_with_tag(
                    request.text,
    ...
    async def synthesize_stream(
        ...
        try:
            ...
            logger.info("Orpheus stream cancelled — closing httpx connection")
            raise
        except Exception as e:
