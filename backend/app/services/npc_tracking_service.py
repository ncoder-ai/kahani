...
class NPCTrackingService:
    ...
    async def _extract_npcs_with_llm_batch(
        ...
        try:
            ...
            # Prompts now live in prompts.yml (npc_tracking_batch).
            prompt = prompt_manager.get_prompt(
                "npc_tracking_batch", "user",
                batch_content=batch_content,
                explicit_names_str=explicit_names_str,
            )
            system_prompt = prompt_manager.get_prompt("npc_tracking_batch", "system") \
                or "You are an expert at analyzing narrative text and extracting named entities. Classify each entity as CHARACTER or ENTITY and provide descriptions."
    ...
    async def _extract_npcs_with_llm(
        ...
        try:
            ...
            # Prompts now live in prompts.yml (npc_tracking_single).
            prompt = prompt_manager.get_prompt(
                "npc_tracking_single", "user",
                scene_sequence=scene_sequence,
                scene_content=scene_content,
                explicit_names_str=explicit_names_str,
            )
            system_prompt = prompt_manager.get_prompt("npc_tracking_single", "system") \
                or "You are a precise story analysis assistant. Extract NPCs and named entities from scenes. Classify each as CHARACTER or ENTITY and provide descriptions. Return only valid JSON."
    ...
    async def _extract_npc_profile(
        ...
        try:
            ...
            # Prompts now live in prompts.yml (npc_character_profile).
            prompt = prompt_manager.get_prompt(
                "npc_character_profile", "user",
                character_name=character_name,
                scenes_text=scenes_text,
            )
            system_prompt = prompt_manager.get_prompt("npc_character_profile", "system") \
                or "You are an expert literary analyst. Extract comprehensive character profiles from story scenes. Return only valid JSON."
