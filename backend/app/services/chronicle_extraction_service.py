...
async def generate_character_snapshot(
    ...
    # Build a proper scene-generation context from `up_to_story_id` so the
    # cache-friendly prefix has the same shape as a real scene gen. Reusing
    # ContextManager here means the system prompt + STORY FOUNDATION +
    # CHARACTER sections all overlap with what the user's scene-gen calls
    # send, so the main LLM can hit cache on the prefix. An empty context
    # produces a malformed prompt that Gemma-class models return empty on.
    from .context_manager import ContextManager
    ctx_mgr = ContextManager(user_settings=user_settings, user_id=user_id)
    scene_generation_context = await ctx_mgr.build_story_context(
        story_id=up_to_story_id,
        db=db,
        branch_id=branch_id,
    )
