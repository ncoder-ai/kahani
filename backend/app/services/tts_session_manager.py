...
class TTSSession(BaseModel):
    ...
    is_cancelled: bool = False  # Poll-based fallback flag (kept for compat)
    # Active asyncio.Task running the generation. cancel_session() calls
    # task.cancel() on this, which propagates CancelledError through the
    # provider's async generators — they re-raise, the async with httpx
    # blocks tear down the upstream connection, and the upstream TTS
    # server detects the disconnect and frees its GPU.
...
class TTSSessionManager:
    ...
    def cancel_session(self, session_id: str):
        """Mark a session as cancelled and actively interrupt its asyncio
        task so generation stops within ~one scheduling tick rather than
        ...
        # Active task cancel: propagates CancelledError into providers,
        # which re-raise so `async with httpx.AsyncClient` blocks close
        # the upstream connection cleanly.
        task = session.generation_task
        if task is not None and not task.done():
            logger.info(f"Session {session_id} cancelling generation_task")
            task.cancel()
        else:
            logger.info(f"Session {session_id} marked cancelled (no active task)")
