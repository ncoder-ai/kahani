...
class ChapterProgressService:
    ...
    def update_plot_progress_from_batches(self, chapter_id: int) -> None:
        ...
        # Flush pending deletes/changes so the query below sees the post-delete
        # state. SessionLocal is configured with autoflush=False (see database.py),
        # so without this flush, just-deleted batches would still appear in the
        # query result and the watermark would be recomputed against stale data.
        self.db.flush()
