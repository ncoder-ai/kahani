...
class ExtractionLLMService:
    ...
    async def extract_plot_events(
        ...
        Args:
            ...
            system_prompt: Optional override; if None, pulled from
                prompts.yml `plot_events_extraction.system`.
        ...
        try:
            ...
            # Pull both prompts from prompts.yml (single source of truth).
            # Fall back to a minimal one-liner only if YAML lookup fails so the
            # extraction still produces an answer instead of crashing.
            if system_prompt is None:
                system_prompt = prompt_manager.get_prompt("plot_events_extraction", "system") \
                    or "You are an expert story analyst skilled at identifying plot events, narrative threads, and story structure."
            prompt = prompt_manager.get_prompt(
                "plot_events_extraction", "user",
                scene_word="scene",
    ...
    async def extract_character_moments(
        ...
        Args:
            ...
            system_prompt: Optional override; if None, pulled from prompts.yml
                `character_moments_extraction.system`.
        ...
        try:
            ...
            if system_prompt is None:
                system_prompt = prompt_manager.get_prompt("character_moments_extraction", "system") \
                    or "You are an expert literary analyst skilled at identifying significant character moments and development in narratives."
            prompt = prompt_manager.get_prompt(
                "character_moments_extraction", "user",
                scene_word="scene",
    ...
    async def extract_npcs(
        ...
        """
        Extract NPCs from a scene (NAMED PEOPLE only, with few-shot example).

        Args:
            scene_content: Scene text to analyze
            scene_sequence: Scene sequence number
            explicit_character_names: List of characters already tracked
            system_prompt: Optional override; if None, pulled from prompts.yml
                `npc_extraction_single.system`.
        ...
        try:
            ...
            if system_prompt is None:
                system_prompt = prompt_manager.get_prompt("npc_extraction_single", "system") \
                    or "You are a precise story analysis assistant. Extract NPCs and named entities from scenes. Return only valid JSON."
            prompt = prompt_manager.get_prompt(
                "npc_extraction_single", "user",
                scene_sequence=scene_sequence,
                scene_content=scene_content,
                explicit_names_str=explicit_names_str,
            )
    ...
    async def extract_npcs_batch(
        ...
        """
        Extract NPCs from multiple scenes in batch (NAMED PEOPLE only).

        Args:
            batch_content: Multiple scenes text (with scene markers)
            explicit_character_names: List of characters already tracked
            system_prompt: Optional override; if None, pulled from prompts.yml
                `npc_extraction_batch.system`.
        ...
        try:
            ...
            if system_prompt is None:
                system_prompt = prompt_manager.get_prompt("npc_extraction_batch", "system") \
                    or "You are a precise story analysis assistant. Extract NPCs and named entities from scenes. Return only valid JSON."
            prompt = prompt_manager.get_prompt(
                "npc_extraction_batch", "user",
...
Include "interactions": [] in your JSON response (empty array if no tracked interactions detected)."""
    ...
    async def extract_character_details(
        ...
        try:
            ...
            if system_prompt is None:
                system_prompt = prompt_manager.get_prompt("character_details_extraction", "system") \
                    or "You are a detailed character profiling assistant. Analyze all appearances of a character in a story and extract comprehensive details about them. Be thorough but accurate - only include information that is clearly supported by the text. Return ONLY valid JSON, no other text."
            prompt = prompt_manager.get_prompt(
                "character_details_extraction", "user",
    ...
    async def generate_summary(
        ...
        try:
            ...
            # Build user prompt — context_section is the caller-side string fragment.
            context_section = f"Story Context:\n{story_context}\n\n" if story_context else ""

            if system_prompt is None:
                system_prompt = prompt_manager.get_prompt("story_summary_extraction", "system") \
                    or "You are an expert story analyst. Create a comprehensive summary that captures the essential plot points and character development while maintaining the tone and style of the original story."
            prompt = prompt_manager.get_prompt(
                "story_summary_extraction", "user",
