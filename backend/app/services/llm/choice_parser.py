...
def fix_single_quotes_to_double(text: str) -> str:
    """
    Convert ['choice', 'another'] to ["choice","another"] for json.loads.

    Replaces single quotes ONLY at array boundaries (right after `[` / `,`
    or right before `,` / `]`), so apostrophes inside strings (possessives
    like "Sara's") are preserved untouched. Once converted, the inner
    apostrophes are valid inside JSON's double-quoted strings.

    Also unwraps single-quoted dict keys like {'choices': ...} → {"choices": ...}
    so the dict-wrapper variant of the same malformation parses too.

    Returns the original text unchanged if it doesn't look like a single-
    quoted array.
    """
...
def extract_choices_with_regex(text: str) -> Optional[List[str]]:
    ...
    # IMPORTANT: anchor on array boundaries — opening quote must follow `[` or `,`
    # (or start of line), closing quote must precede `,` or `]` (or end of line).
    # Without this anchoring, the regex matches the SPAN BETWEEN two apostrophes
    # inside a possessive (e.g. "Sara's car was Marcus's pride and joy") and
    # silently truncates it to "s car was Marcus".
    if len(choices) < 2:
...
def parse_choices_from_json(text: str) -> Optional[List[str]]:
    ...
    try:
        ...
        # === STRATEGY 4: Single-quoted array with embedded apostrophes ===
        # OpenRouter / some models emit ['Sara's car', 'Marcus's bag'] which is
        # invalid JSON because the inner apostrophes look like string delimiters.
        # Convert only the boundary single-quotes to double-quotes, then parse.
        sq_converted = fix_single_quotes_to_double(text)
        if sq_converted != text:
            try:
                parsed = json.loads(sq_converted)
                if isinstance(parsed, dict) and "choices" in parsed:
                    result = _validate_and_return_choices(parsed["choices"])
                    if result:
                        logger.debug("[CHOICES PARSE] Succeeded after single-quote → double-quote conversion (dict)")
                        return result
                elif isinstance(parsed, list):
                    result = _validate_and_return_choices(parsed)
                    if result:
                        logger.debug("[CHOICES PARSE] Succeeded after single-quote → double-quote conversion (array)")
                        return result
            except json.JSONDecodeError:
                pass

        # === STRATEGY 5: Regex fallback - extract quoted strings directly ===
