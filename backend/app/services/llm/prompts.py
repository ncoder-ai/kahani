...
class PromptManager:
    ...
    def get_raw_prompt(self, template_key: str, **template_vars) -> str:
        ...
        yaml_mapping = {
            ...
            # TTS segment extraction (Phase B+C of immersive TTS)
            "tts.segment_extraction": ("tts", "segment_extraction"),
            # System prompts for service.py _build_simple_extraction_messages call sites
            # (previously hardcoded; see docs/EXTRACTION_ISSUES.md §1b)
            "simple_extraction_systems.plot_extraction": ("simple_extraction_systems", "plot_extraction"),
            "simple_extraction_systems.combined_extraction": ("simple_extraction_systems", "combined_extraction"),
            "simple_extraction_systems.combined_extraction_batch": ("simple_extraction_systems", "combined_extraction_batch"),
            "simple_extraction_systems.events_and_npcs": ("simple_extraction_systems", "events_and_npcs"),
            "simple_extraction_systems.scene_event_extraction": ("simple_extraction_systems", "scene_event_extraction"),
            "simple_extraction_systems.chapter_summary_cache_friendly": ("simple_extraction_systems", "chapter_summary_cache_friendly"),
            "simple_extraction_systems.entity_states_cache_friendly": ("simple_extraction_systems", "entity_states_cache_friendly"),
            "simple_extraction_systems.relationship_extraction_cache_friendly": ("simple_extraction_systems", "relationship_extraction_cache_friendly"),
            "simple_extraction_systems.working_memory_cache_friendly": ("simple_extraction_systems", "working_memory_cache_friendly"),
            "simple_extraction_systems.scene_summary_for_embedding": ("simple_extraction_systems", "scene_summary_for_embedding"),
            "simple_extraction_systems.npc_extraction_cache_friendly": ("simple_extraction_systems", "npc_extraction_cache_friendly"),
            "simple_extraction_systems.character_moments_cache_friendly": ("simple_extraction_systems", "character_moments_cache_friendly"),
            "simple_extraction_systems.plot_events_cache_friendly": ("simple_extraction_systems", "plot_events_cache_friendly"),
            "simple_extraction_systems.chronicle_extraction": ("simple_extraction_systems", "chronicle_extraction"),
            "simple_extraction_systems.chronicle_validation": ("simple_extraction_systems", "chronicle_validation"),
            "simple_extraction_systems.character_snapshot_generation": ("simple_extraction_systems", "character_snapshot_generation"),
        }
    ...
    def get_task_instruction(self, has_immediate: bool, prose_style: str = 'balanced', tone: str = None, scene_focus: str = None, narrative_presence_active: bool = False, **template_vars) -> str:
        """
        ...
        scene_focus_reminder = self.get_scene_focus_reminder(scene_focus)
        narrative_presence_nudge = self.get_narrative_presence_nudge(narrative_presence_active)
        ...
        template_vars_with_style['scene_focus_reminder'] = scene_focus_reminder
        template_vars_with_style['narrative_presence_nudge'] = narrative_presence_nudge
...
Chapter Conclusion:"""
    ...
    def get_scene_focus_reminder(self, focus_character: Optional[str] = None) -> str:
        """
        Get scene-focus reminder for task instructions.

        Spotlight semantics only: the named character gets the most narrative
        weight without changing the narrative point of view (safe for
        second-person stories). Empty/None produces an empty string so the
        task message is byte-identical to the no-focus case.

        Args:
            focus_character: Name of the character to spotlight, or None

        Returns:
            The scene-focus reminder text, or empty string if no focus
        """
        if not focus_character or not str(focus_character).strip():
            return ""

        name = str(focus_character).strip()
        return (
            f"SCENE FOCUS: Center this scene on {name}. Give {name} the most "
            f"narrative weight — their choices, reactions, and arc should drive "
            f"what happens. Keep other characters present and in-character but "
            f"secondary. Do NOT change the narrative point of view to do this."
        )
    
    # Feature 3 — per-character NARRATIVE PRESENCE. Wording below is the EXACT
    # text validated by backend/scripts/test_narrative_presence.py v3
    # (quiet-scene interiority A/B + inversion control, 16-gen, 2026-05-16).
    # "bias, never a hard zero": v2 `spoken` zeroed interiority and `internal`
    # only suppressed speech; v3 makes expression a bias and makes a quiet
    # scene the place interiority FILLS rather than gets skipped. Do not
    # casually reword without re-running the harness.
    _NP_PRESENCE_PHRASE = {
        "low": "a minor presence — keep them mostly in the background with little focus",
        "high": "a prominent presence — the scene gives them substantial real estate "
                "(focus, and per their expression below, either dialogue/action or interiority)",
        # "normal"/None -> no phrase (no-op)
    }
    _NP_EXPRESSION_PHRASE = {
        "internal": "Weight that real estate heavily toward their inner life: give them "
                    "frequent, substantial internal monologue and reflection. They speak "
                    "less than others. In quiet or still moments their interiority should "
                    "carry the scene — that is when we go deepest into their head, not when "
                    "we skip past them",
        "spoken": "Weight that real estate toward dialogue and outward action; keep their "
                  "introspection lighter and occasional, but never absent — even a "
                  "dialogue-forward character has a few clear inner beats",
        # "balanced"/None -> no phrase (no-op)
    }
    _NP_NUDGE = (
        "IMPORTANT: Do NOT distribute dialogue and interiority evenly. Weight BOTH how "
        "much each character speaks AND how much we see their thoughts by their NARRATIVE "
        "PRESENCE and expression above. Internal characters get frequent, substantial "
        "inner monologue; spoken characters lead the dialogue but still get occasional "
        "thoughts; low-presence characters recede. Critically: a quiet or low-action "
        "stretch is NOT a gap to skim — it is exactly where the prominent/internal "
        "characters' inner lives should fill the page. Never round-robin; an uneven, "
        "channel-appropriate distribution is the goal."
    )

    def get_narrative_presence_instruction(self, np: Optional[Dict[str, Any]]) -> str:
        """Per-character cached line for the CHARACTER DIALOGUE STYLES message.

        np = {"presence": "low"|"normal"|"high", "expression": "internal"|"balanced"|"spoken"}
        Returns '' for None or the default (normal+balanced) so an unconfigured
        character produces a byte-identical cached prefix.
        """
        if not np or not isinstance(np, dict):
            return ""
        p = self._NP_PRESENCE_PHRASE.get(np.get("presence") or "normal")
        e = self._NP_EXPRESSION_PHRASE.get(np.get("expression") or "balanced")
        parts = [s for s in (p, e) if s]
        if not parts:
            return ""
        return "NARRATIVE PRESENCE: " + ". ".join(s[0].upper() + s[1:] for s in parts) + "."

    def get_narrative_presence_nudge(self, active: bool = False) -> str:
        """Global anti-round-robin nudge for the task message (post-cache-break,
        same slot as scene_focus_reminder). Empty unless at least one in-scene
        character has a non-default presence, so non-users get a byte-identical
        task message."""
        return self._NP_NUDGE if active else ""
    ...
    def _get_yaml_prompt(self, template_key: str, prompt_type: str) -> str:
        ...
        yaml_mapping = {
            ...
            # Plot events extraction (direct path, not cache-friendly)
            "plot_events_extraction": ("plot_events_extraction", ""),
            # Character moments extraction (direct path, not cache-friendly)
            "character_moments_extraction": ("character_moments_extraction", ""),
            # NPC extraction (direct path, not cache-friendly). 5 templates:
            "npc_extraction_single": ("npc_extraction_single", ""),
            "npc_extraction_batch": ("npc_extraction_batch", ""),
            "npc_tracking_single": ("npc_tracking_single", ""),
            "npc_tracking_batch": ("npc_tracking_batch", ""),
            "npc_character_profile": ("npc_character_profile", ""),
            # Character details + summary (extraction_service.py standalone methods)
            "character_details_extraction": ("character_details_extraction", ""),
            "story_summary_extraction": ("story_summary_extraction", ""),
            # Interaction extraction (background_tasks.py)
            "interaction_extraction": ("interaction_extraction", ""),
            ...
            "relationship_extraction_cache_friendly": ("relationship_extraction_cache_friendly", ""),
