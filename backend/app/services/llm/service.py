...
class UnifiedLLMService:
    ...
    async def generate_for_task(
        ...
        if task_type in ("extraction", "agent"):
            ...
            # Respect user's configured reasoning_effort — don't override
    ...
    @staticmethod
    def _narrative_presence_active(context: Dict[str, Any]) -> bool:
        """True if any in-scene character has a non-default narrative_presence
        (Feature 3). Gates the global anti-round-robin nudge so a story not
        using the feature gets a byte-identical task message."""
        chars = context.get("characters")
        if isinstance(chars, dict):
            lst = chars.get("active_characters", []) + chars.get("inactive_characters", [])
        elif isinstance(chars, list):
            lst = chars
        else:
            return False
        for c in lst:
            np = c.get("narrative_presence") if isinstance(c, dict) else None
            if np and isinstance(np, dict) and (
                (np.get("presence") or "normal") != "normal"
                or (np.get("expression") or "balanced") != "balanced"
            ):
                return True
        return False

    @staticmethod
    def _save_prompt_snapshot(context: Dict[str, Any], messages: List[Dict[str, str]]) -> None:
        """Save the assembled cache-friendly prompt (prefix + task) onto context.

        Read by SceneVariantServiceAdapter (story_helpers.py) and persisted on
        `scene_variants.context_snapshot`. Variant regeneration loads it via
        `_saved_prompt_prefix` so the new LLM call replays the exact original
        messages — preserving the main LLM's KV cache across regenerations.

        Call at the end of every cache-friendly generation method, right after
        assembling `messages` and before the LLM call.
        """
    ...
    def _build_scene_task_message(
        ...
        task_content = prompt_manager.get_task_instruction(
            ...
            narrative_presence_active=self._narrative_presence_active(context),
...
Chapter Conclusion:"""
    ...
    async def generate_scene(self, context: Dict[str, Any], user_id: int, user_settings: Dict[str, Any], db: Optional[Session] = None) -> str:
        ...
        # Save full prompt snapshot for variant regeneration (see CLAUDE.md
        # "Cache-Optimized Message Ordering"). Variant gen reads this from
        # SceneVariant.context_snapshot to reuse the exact same prefix.
        self._save_prompt_snapshot(context, messages)
    ...
    async def generate_scene_with_choices(self, context: Dict[str, Any], user_id: int, user_settings: Dict[str, Any], db: Optional[Session] = None) -> Tuple[str, Optional[List[str]]]:
        ...
        # Save full prompt snapshot for variant regeneration — same pattern as
        # the streaming sibling at generate_scene_with_choices_streaming.
        self._save_prompt_snapshot(context, messages)
    ...
    async def generate_variant_with_choices(
        ...
        # Save full prompt snapshot for downstream variant regenerations.
        self._save_prompt_snapshot(context, messages)
    ...
    async def generate_continuation_with_choices(
        ...
        # Save full prompt snapshot so downstream variants can reuse the prefix.
        self._save_prompt_snapshot(context, messages)
    ...
    async def generate_scene_streaming(self, context: Dict[str, Any], user_id: int, user_settings: Dict[str, Any]) -> AsyncGenerator[str, None]:
        ...
        # Save full prompt snapshot for downstream variant regeneration.
        self._save_prompt_snapshot(context, messages)
    ...
    async def generate_variant_with_choices_streaming(
        ...
        # Save full prompt snapshot for variant-of-variant cache reuse.
        self._save_prompt_snapshot(context, messages)
...
Chapter Conclusion:"""
    ...
    async def generate_choices(self, scene_content: str, context: Dict[str, Any], user_id: int, user_settings: Dict[str, Any], db: Optional[Session] = None) -> List[str]:
        ...
        # Use user's configured reasoning_effort — don't override it.
        # Some models (e.g. MiniMax) require reasoning and reject overrides with 400.
    ...
    async def extract_plot_events_with_context(
        ...
        if use_extraction_llm or not use_cache:
            messages = self._build_simple_extraction_messages(
                prompt_manager.get_raw_prompt("simple_extraction_systems.plot_extraction")
                    or "You analyze story scenes to identify which planned plot events have occurred. Return only valid JSON.",
                final_message
            )
    ...
    async def extract_combined_cache_friendly(
        ...
        if max_tokens is None:
            max_tokens = prompt_manager.get_max_tokens("combined_extraction", user_settings)
        ...
        if use_extraction_llm or not use_cache:
            messages = self._build_simple_extraction_messages(
                prompt_manager.get_raw_prompt("simple_extraction_systems.combined_extraction")
                    or "You are a precise story analysis assistant. Extract characters, NPCs, plot events, and entity states. Return only valid JSON.",
                final_message
            )
    ...
    async def extract_combined_cache_friendly_batch(
        ...
        if use_extraction_llm or not use_cache:
            messages = self._build_simple_extraction_messages(
                prompt_manager.get_raw_prompt("simple_extraction_systems.combined_extraction_batch")
                    or "You are a precise story analysis assistant. Extract characters, NPCs, plot events, and entity states from scenes. Return only valid JSON.",
                final_message
            )
    ...
    async def extract_events_and_npcs_cache_friendly(
        ...
        if use_extraction_llm or not use_cache:
            messages = self._build_simple_extraction_messages(
                prompt_manager.get_raw_prompt("simple_extraction_systems.events_and_npcs")
                    or "You extract factual events and named NPCs from story scenes. Return only valid JSON.",
                final_message
            )
    ...
    async def extract_scene_events_cache_friendly(
        ...
        When force_main_llm is True, routes to the main LLM even if the
        extraction LLM is enabled — mirrors extract_plot_events_cache_friendly.

        Returns:
            Raw JSON string with {"events": [...]}
        """
        use_extraction_llm = self._will_use_extraction_llm(user_settings, force_main_llm)
        use_cache = user_settings.get('generation_preferences', {}).get('use_cache_friendly_prompts', True)
        if max_tokens is None:
            max_tokens = prompt_manager.get_max_tokens("scene_events", user_settings)
        ...
        if use_extraction_llm or not use_cache:
            messages = self._build_simple_extraction_messages(
                prompt_manager.get_raw_prompt("simple_extraction_systems.scene_event_extraction")
                    or "You extract factual events from story scenes. Return only valid JSON.",
                final_message
            )
    ...
    async def generate_chapter_summary_cache_friendly(
        ...
        use_cache = user_settings.get('generation_preferences', {}).get('use_cache_friendly_prompts', True)
        if max_tokens is None:
            max_tokens = prompt_manager.get_max_tokens("chapter_summary", user_settings)
        ...
        if use_extraction_llm or not use_cache:
            messages = self._build_simple_extraction_messages(
                prompt_manager.get_raw_prompt("simple_extraction_systems.chapter_summary_cache_friendly")
                    or "You are a skilled story analyst. Create concise, accurate chapter summaries that capture key events and character developments.",
                final_message
            )
    ...
    async def extract_entity_states_cache_friendly(
        ...
        use_cache = user_settings.get('generation_preferences', {}).get('use_cache_friendly_prompts', True)
        if max_tokens is None:
            max_tokens = prompt_manager.get_max_tokens("entity_states", user_settings)
        ...
        if use_extraction_llm or not use_cache:
            messages = self._build_simple_extraction_messages(
                prompt_manager.get_raw_prompt("simple_extraction_systems.entity_states_cache_friendly")
                    or "You extract entity states (characters, locations, objects) from story scenes. Return only valid JSON.",
                final_message
            )
    ...
    async def extract_relationship_cache_friendly(
        ...
        use_cache = user_settings.get('generation_preferences', {}).get('use_cache_friendly_prompts', True)
        if max_tokens is None:
            max_tokens = prompt_manager.get_max_tokens("relationship", user_settings)
        ...
        if use_extraction_llm or not use_cache:
            messages = self._build_simple_extraction_messages(
                prompt_manager.get_raw_prompt("simple_extraction_systems.relationship_extraction_cache_friendly")
                    or "You extract relationship changes between characters from story scenes. Return only valid JSON.",
                final_message
            )
    ...
    async def extract_working_memory_cache_friendly(
        ...
        use_cache = user_settings.get('generation_preferences', {}).get('use_cache_friendly_prompts', True)
        if max_tokens is None:
            max_tokens = prompt_manager.get_max_tokens("working_memory", user_settings)
        ...
        if use_extraction_llm or not use_cache:
            messages = self._build_simple_extraction_messages(
                prompt_manager.get_raw_prompt("simple_extraction_systems.working_memory_cache_friendly")
                    or "You extract working memory updates (narrative focus and character spotlight) from story scenes. Return only valid JSON.",
                final_message
            )
            ...
            if json_match:
                try:
                    ...
                    try:
                        ...
                        logger.error(f"[WORKING_MEMORY] JSON parse failed after cleaning: {e2}\n--- RAW ---\n{response_text}\n--- CLEANED ---\n{cleaned}")
                        return None

            logger.error(f"[WORKING_MEMORY] No JSON object in response:\n{response_text}")
            return None
    ...
    async def generate_scene_summary_cache_friendly(
        ...
        if use_extraction_llm or not use_cache:
            messages = self._build_simple_extraction_messages(
                prompt_manager.get_raw_prompt("simple_extraction_systems.scene_summary_for_embedding")
                    or "You generate concise factual summaries and location descriptions for story scenes.",
                final_message
            )
    ...
    async def extract_npcs_cache_friendly(
        ...
        use_cache = user_settings.get('generation_preferences', {}).get('use_cache_friendly_prompts', True)
        if max_tokens is None:
            max_tokens = prompt_manager.get_max_tokens("npc_extraction", user_settings)
        ...
        if use_extraction_llm or not use_cache:
            messages = self._build_simple_extraction_messages(
                prompt_manager.get_raw_prompt("simple_extraction_systems.npc_extraction_cache_friendly")
                    or "You extract named NPCs (non-player characters) from story scenes. Return only valid JSON.",
                final_message
            )
    ...
    async def extract_character_moments_cache_friendly(
        ...
        use_cache = user_settings.get('generation_preferences', {}).get('use_cache_friendly_prompts', True)
        if max_tokens is None:
            max_tokens = prompt_manager.get_max_tokens("character_moments", user_settings)
        ...
        if use_extraction_llm or not use_cache:
            messages = self._build_simple_extraction_messages(
                prompt_manager.get_raw_prompt("simple_extraction_systems.character_moments_cache_friendly")
                    or "You extract significant character moments from story scenes. Return only valid JSON.",
                final_message
            )
    ...
    async def extract_plot_events_fallback_cache_friendly(
        ...
        use_cache = user_settings.get('generation_preferences', {}).get('use_cache_friendly_prompts', True)
        if max_tokens is None:
            max_tokens = prompt_manager.get_max_tokens("plot_extraction", user_settings)
        ...
        if use_extraction_llm or not use_cache:
            messages = self._build_simple_extraction_messages(
                prompt_manager.get_raw_prompt("simple_extraction_systems.plot_events_cache_friendly")
                    or "You extract significant plot events from story scenes. Return only valid JSON.",
                final_message
            )
    ...
    async def extract_chronicle_cache_friendly(
        ...
        use_cache = user_settings.get('generation_preferences', {}).get('use_cache_friendly_prompts', True)
        if max_tokens is None:
            max_tokens = prompt_manager.get_max_tokens("chronicle_extraction", user_settings)
        ...
        if use_extraction_llm or not use_cache:
            messages = self._build_simple_extraction_messages(
                prompt_manager.get_raw_prompt("simple_extraction_systems.chronicle_extraction")
                    or "You extract character chronicle entries and location lorebook events from story scenes. Return only valid JSON.",
                final_message
            )
    ...
    async def validate_chronicle_cache_friendly(
        ...
        use_cache = user_settings.get('generation_preferences', {}).get('use_cache_friendly_prompts', True)
        if max_tokens is None:
            max_tokens = prompt_manager.get_max_tokens("chronicle_validation", user_settings)
        ...
        if use_extraction_llm or not use_cache:
            messages = self._build_simple_extraction_messages(
                prompt_manager.get_raw_prompt("simple_extraction_systems.chronicle_validation")
                    or "You validate chronicle and lorebook entries against story scene content. Return only valid JSON.",
                final_message
            )
    ...
    async def generate_snapshot_cache_friendly(
        ...
        Args:
            force_main_llm: When True, the call ALWAYS routes to the main LLM
                (e.g. world-level admin endpoints). The simple/cache-friendly
                message shape still follows the standard rule below.

        Routing (regardless of force_main_llm):
            - use_cache=True  → cache-friendly multi-message prefix when going
              to the main LLM (force_main_llm=True OR extraction LLM disabled).
              Otherwise (extraction LLM target) the simple 2-message form is
              used — Ministral's template rejects multiple user messages.
            - use_cache=False → simple 2-message form always.
        ...
        use_cache = user_settings.get('generation_preferences', {}).get('use_cache_friendly_prompts', True)
        if max_tokens is None:
            max_tokens = prompt_manager.get_max_tokens("snapshot", user_settings)
        ...
        if use_extraction_llm or not use_cache:
            messages = self._build_simple_extraction_messages(
                prompt_manager.get_raw_prompt("simple_extraction_systems.character_snapshot_generation")
                    or "You generate concise character snapshot paragraphs that summarize a character's current state based on chronicle entries.",
                final_message
            )
    ...
    async def regenerate_scene_variant(
        ...
        # Pass the scene's chapter_id so the character listing renders with
        # the same active/inactive tiering that new-scene generation uses.
        # Without this, build_scene_generation_context falls to the flat-list
        # branch (context_manager.py:432) and produces a byte-divergent prefix.
        effective_chapter_id = scene.chapter_id
        ...
        if custom_prompt:
            context = await context_manager.build_scene_generation_context(
                ...
                chapter_id=effective_chapter_id, branch_id=effective_branch_id,
            ...
            context = await context_manager.build_scene_generation_context(
                story.id, db, exclude_scene_id=scene_id,
                chapter_id=effective_chapter_id, branch_id=effective_branch_id,
                is_variant_generation=True
            )

        # Cache-friendliness: load the original variant's saved prompt prefix.
        # When present, `_build_cache_friendly_message_prefix` short-circuits
        # and returns this verbatim, guaranteeing the variant prompt has a
        # byte-identical prefix to the original scene's prompt for max
        # KV-cache hit. Mirrors the streaming variant endpoint at
        # variant_endpoints.py:497-543. Falls through to fresh-context build
        # if the original variant has no snapshot (legacy scenes).
        if original_variant and original_variant.context_snapshot:
            try:
                import json as _json
                raw = _json.loads(original_variant.context_snapshot)
                if isinstance(raw, dict) and raw.get("v") == 2:
                    snapshot_messages = raw["messages"]
                    # Strip the original task message; the variant task is appended
                    # later by generate_variant_with_choices.
                    context["_saved_prompt_prefix"] = snapshot_messages[:-1]
                    logger.info(
                        f"[VARIANT NON-STREAMING] Loaded prompt snapshot from variant "
                        f"{original_variant.id} ({len(snapshot_messages)} messages)"
                    )
            except (ValueError, TypeError) as e:
                logger.warning(f"[VARIANT NON-STREAMING] Failed to parse context_snapshot: {e}")
        ...
        # Create the new variant. Carry over the snapshot from the original so
        # subsequent regenerations of this variant can still cache-hit. Also
        # persist any newly-built snapshot if generate_variant_with_choices set one.
        snapshot_for_variant = (
            context.get('_prompt_prefix_snapshot')
            or (original_variant.context_snapshot if original_variant else None)
        )
        variant = SceneVariant(
            ...
            context_snapshot=snapshot_for_variant,
        )
    ...
    async def _generate_with_messages(
        ...
        # Apply API-specific thinking disable params for main LLM. Each
        # param is provider-specific — passing thinking_config to a non-gemini
        # provider, prompt_mode to non-mistral, etc. crashes the underlying
        # SDK with "unexpected keyword argument". Gate by client.api_type so
        # a stale thinking_model_type setting doesn't break generation when
        # the user switches providers.
        api_type = (client.api_type or "").lower()
        if thinking_model_type != 'none' and not thinking_enabled_gen:
            if thinking_model_type == 'mistral' and api_type == 'mistral':
                gen_params["prompt_mode"] = None
            elif thinking_model_type == 'openai' and api_type == 'openai':
                gen_params["reasoning_effort"] = "none"
            elif thinking_model_type == 'gemini' and api_type in ('gemini', 'vertex_ai', 'google'):
                gen_params["thinking_config"] = {"thinking_budget": 0}
            elif thinking_model_type == 'glm':
                # GLM models commonly run on vLLM/openai-compatible endpoints,
                # so the chat_template_kwargs ride along inside extra_body
                # which most providers ignore. Safe to apply unconditionally.
                extra = gen_params.get("extra_body", {})
                extra["chat_template_kwargs"] = {"enable_thinking": False}
                gen_params["extra_body"] = extra
            elif api_type == 'openai-compatible':
                # Best-effort fallback for any local openai-compatible server
                # (vLLM, llama.cpp, koboldcpp, ik_llama.cpp) when the user's
                # thinking_model_type doesn't match a provider-specific branch
                # above. The GLM-style chat_template_kwargs.enable_thinking key
                # is the most widely-respected disable knob across local
                # backends — vLLM honors it for Qwen/GLM, koboldcpp ignores it
                # silently, and llama.cpp's Gemma path also ignores it (use
                # `--reasoning off` at server launch for Gemma instead). Ignored
                # keys do not crash the server, so this is safe to send.
                extra = gen_params.get("extra_body", {})
                extra.setdefault("chat_template_kwargs", {})["enable_thinking"] = False
                gen_params["extra_body"] = extra
                logger.debug(
                    f"[THINKING] Applied GLM-style fallback for "
                    f"thinking_model_type={thinking_model_type!r} on openai-compatible server"
                )
            else:
                logger.debug(
                    f"[THINKING] Skipping {thinking_model_type}-specific param "
                    f"(client api_type={api_type!r} doesn't match)"
                )
    ...
    async def _generate_stream_with_messages(
        ...
        # Apply API-specific thinking disable params for main LLM. See the
        # non-streaming path above for the full rationale — gate by
        # client.api_type so stale thinking_model_type settings don't crash
        # streaming generation when the user switches providers.
        api_type = (client.api_type or "").lower()
        if thinking_model_type != 'none' and not thinking_enabled_gen:
            if thinking_model_type == 'mistral' and api_type == 'mistral':
                gen_params["prompt_mode"] = None
                ...
                logger.debug(
                    f"[THINKING:STREAM] Skipping {thinking_model_type}-specific param "
                    f"(client api_type={api_type!r} doesn't match)"
                )
