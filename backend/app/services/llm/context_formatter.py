...
def format_character_voice_styles(characters: Any) -> str:
    ...
    for char in char_list:
        ...
        if char.get('voice_style'):
            voice_instruction = prompt_manager.get_voice_style_instruction(char['voice_style'])
            if voice_instruction:
                segments.append(voice_instruction)
        # Feature 3: per-character narrative presence (same cached message;
        # '' for unconfigured/default so the cached prefix stays byte-identical)
        np_instruction = prompt_manager.get_narrative_presence_instruction(char.get('narrative_presence'))
        if np_instruction:
            segments.append(np_instruction)
        if segments:
            voice_styles.append(f"**{char.get('name', 'Unknown')}**: " + " ".join(segments))
