...
def _coerce_field_to_string(value: Any) -> str:
    """
    Coerce an LLM-returned value into a readable string for storage.

    Small extraction LLMs (Ministral 3B / 8B, Qwen 2.5 3B) ignore the prompt
    instruction to return flat strings and emit nested objects for fields
    like fears, goals, appearance, background. Stringifying a dict with
    Python's `str()` produces ugly repr ({'key': 'value'}) which surfaces
    in the UI as raw JSON-looking text. This helper flattens whatever shape
    the model returned into prose-y text.

    Rules:
    - None / falsy → empty string
    - str → returned as-is, stripped
    - list → "; "-joined after coercing each element
    - dict → "Key: value; Key: value" (keys title-cased, underscores → spaces)
            recursively coerces nested dict/list values
    - other → str() fallback (last resort)
    """
    if value is None:
        return ''
    ...
    if isinstance(value, list):
        return '; '.join(_coerce_field_to_string(item) for item in value if item is not None)
    if isinstance(value, dict):
        parts: List[str] = []
        for key, sub in value.items():
            label = str(key).replace('_', ' ').strip()
            label = label[:1].upper() + label[1:] if label else ''
            sub_text = _coerce_field_to_string(sub)
            if sub_text:
                parts.append(f"{label}: {sub_text}" if label else sub_text)
        return '; '.join(parts)
    return str(value)
...
class CharacterAssistantService:
    ...
    def _normalize_character_details(self, character_details: Dict[str, Any], character_scenes: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        # Small extraction LLMs (Ministral / Qwen-3B class) sometimes ignore the
        # "flat strings" instruction in the prompt and return nested objects for
        # background / goals / fears / appearance — e.g. {"fears": {"deepest_fear":
        # "...", "phobias": "..."}}. str(dict) on those produces Python-repr
        # strings ("{'deepest_fear': '...'}") which surface in the UI as raw JSON.
        # Coerce here so storage and display see human-readable text regardless of
        # how the model decided to shape the response.
        for field in ('goals', 'fears', 'background', 'appearance'):
            if field in character_details:
                character_details[field] = _coerce_field_to_string(character_details[field])

        # Ensure other string fields are strings
        for field in ['name', 'description', 'suggested_role']:
