...
_FOCUS_MARKDOWN_GUNK_RE = re.compile(r"\*+['‘’“”\"]?")


def _coerce_focus_item(item: Any) -> str:
    """Coerce a working memory focus item to a clean plain string.

    The LLM sometimes returns dicts (e.g. {"type": ..., "description": ...})
    instead of strings. We extract the description, then strip markdown gunk
    (asterisks, smart quotes) that snowballs when re-fed into the next prompt.
    """
    if not item:
        return ""
    if isinstance(item, dict):
        text = (item.get("description") or item.get("text") or item.get("focus")
                or item.get("item") or item.get("content") or "")
        if not text and item:
            text = " | ".join(str(v) for v in item.values() if isinstance(v, str))
    else:
        text = str(item)
    text = _FOCUS_MARKDOWN_GUNK_RE.sub("", text)
...
class EntityStateService:
    ...
    async def update_working_memory(
        ...
        try:
            ...
            # Get current state for context — sanitize on read so legacy corrupted
            # entries (stringified dicts, markdown gunk) don't poison the prompt
            current_focus = [_coerce_focus_item(f) for f in (memory.recent_focus or [])]
            current_focus = [f for f in current_focus if f]
            ...
            if updates:
                ...
                memory.recent_focus = [_coerce_focus_item(f) for f in raw_focus if _coerce_focus_item(f)]
