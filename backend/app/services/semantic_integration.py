...
async def cleanup_scene_embeddings(scene_id: int, db: Session):
    ...
    try:
        ...
        # Roll back chapter watermarks so the next threshold sweep re-extracts
        # this scene. Without this, the rows wiped above stay empty forever
        # because the watermark still says this scene was processed.
        # Mirrors the pattern in scene_database_operations.py:621 (scene-delete path).
        # "Only lower, never raise" — safe under concurrent cleanup for different scenes.
        #
        # Direct SQL UPDATE (synchronize_session=False) so we don't touch the
        # ORM identity map. Earlier in this request lifecycle
        # invalidate_plot_progress_batches_for_scene may have committed a new
        # last_plot_extraction_scene_count via the ORM; expiring its Chapter
        # object here would otherwise force a session refresh that could
        # re-emit a stale UPDATE on db.commit() below.
        scene = db.query(Scene).filter(Scene.id == scene_id).first()
        if scene and scene.chapter_id:
            from sqlalchemy import update
            rollback_seq = max(0, scene.sequence_number - 1)
            db.execute(
                update(Chapter)
                .where(Chapter.id == scene.chapter_id)
                .where(Chapter.last_extraction_scene_count > rollback_seq)
                .values(last_extraction_scene_count=rollback_seq),
                execution_options={"synchronize_session": False},
            )
            db.execute(
                update(Chapter)
                .where(Chapter.id == scene.chapter_id)
                .where(Chapter.last_chronicle_scene_count > rollback_seq)
                .values(last_chronicle_scene_count=rollback_seq),
                execution_options={"synchronize_session": False},
            )
