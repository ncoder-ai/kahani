...
async def run_recall_agent(
    ...
    try:
        ...
        min_scenes = agent_config.get("min_scenes", 10)

        # Pre-check: stories with very few scenes have nothing meaningful to
        # recall from. Running the agent against an effectively-empty event
        # index just burns Ministral round-trips (5s/turn) until max_turns,
        # adding 30-40s of latency. The deterministic single-query path still
        # runs as fallback and gives the baseline RELATED PAST SCENES context.
        from ...models.scene import Scene
        scene_count = db.query(Scene).filter(
            Scene.story_id == story_id,
            Scene.is_deleted == False,  # noqa: E712 — SQLAlchemy comparator
            *([Scene.branch_id == branch_id] if branch_id else [Scene.branch_id.is_(None)]),
        ).count()
        if scene_count < min_scenes:
            logger.info(
                f"[RECALL AGENT] Skipped: story has {scene_count} scenes < min_scenes={min_scenes} "
                f"(deterministic pipeline will run as fallback)"
            )
            return None
