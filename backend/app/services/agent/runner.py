...
# Observation prefixes/substrings that indicate the tool found nothing.
# Used to bail early when the agent keeps querying with no signal — without
# this, the agent loops to max_turns burning ~5s per LLM call against
# Ministral while the user waits.
_EMPTY_OBSERVATION_MARKERS = (
    "No matching events found.",
    "No scenes found matching that query.",
    "No scenes found near sequence",
    "has no scenes.",
)

# Bail after this many consecutive empty observations.
EMPTY_OBSERVATION_BAIL = 2
...
class AgentRunner:
    ...
    async def run(self, user_message: str) -> Dict[str, Any]:
        ...
        for turn in range(self.max_turns):
            ...
            # Track consecutive empty observations and bail when the agent
            # is clearly spinning. Don't count tool errors — those are
            # potentially transient and the agent might recover.
            if any(marker in observation for marker in _EMPTY_OBSERVATION_MARKERS):
                consecutive_empty += 1
                if consecutive_empty >= EMPTY_OBSERVATION_BAIL:
                    error = (
                        f"Agent bailed: {consecutive_empty} consecutive empty observations "
                        f"(turn {turn + 1}/{self.max_turns})"
                    )
                    logger.info(f"[{self.agent_name}] {error}")
                    if self.trace_logger:
