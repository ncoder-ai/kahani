...
async def websocket_tts_stream(
    ...
    try:
        ...
        # Cancel any in-flight generation before tearing down the session,
        # so the upstream TTS server's GPU isn't held by an orphaned task
        # whose response no longer has a consumer.
        tts_session_manager.cancel_session(session_id)
