...
def _resolve_request_provider(request, user_settings) -> tuple:
    """Resolve (api_type, api_key, api_url) for a model-fetch / test-connection request.

    Multi-instance aware: if request.provider_id is set and the slug exists in
    configured_providers, the entry's fields take precedence. Falls back to
    request.api_type as a slug (legacy single-instance mode), then to the
    request's explicit api_url/api_key fields.
    """
    api_type = None
    api_key = None
    api_url = None

    # Step 1: explicit provider_id (multi-instance mode)
    if user_settings and getattr(request, "provider_id", None):
        entry = user_settings._get_provider_entry(request.provider_id)
        if entry:
    ...
    api_type = api_type or request.api_type or "openai_compatible"
    if user_settings and (not api_key or not api_url):
    ...
    # Step 3: explicit request fields override empties
    api_key = api_key or request.api_key or ""
    api_url = api_url or request.api_url or ""

    return api_type, api_key, api_url
...
class LLMSettingsUpdate(BaseModel):
    ...
    api_url: Optional[str] = None  # Legacy: only used when provider_id not set
    api_key: Optional[str] = None
    api_type: Optional[str] = Field(default="openai_compatible")  # Legacy: kept in flat column for fallback
    model_name: Optional[str] = None
    provider_id: Optional[str] = None  # NEW: slug into configured_providers — preferred over api_type when set
...
class STTSettingsUpdate(BaseModel):
    ...
    # BCP-47 locale or empty string for system default. Validated loosely —
    # frontend offers a curated dropdown; this regex just blocks junk.
...
class ExtractionModelSettingsUpdate(BaseModel):
    ...
    api_key: Optional[str] = None
    model_name: Optional[str] = None
    api_type: Optional[str] = None  # Legacy: kept in flat column for fallback
    provider_id: Optional[str] = None  # NEW: slug into configured_providers — preferred over api_type when set
    ...
    use_main_llm_for_scene_events: Optional[bool] = None  # Use main LLM for per-scene event extraction (recall index)
...
class EmbeddingModelSettingsUpdate(BaseModel):
    ...
    provider: Optional[str] = None  # Legacy: kept in flat column for fallback
    api_url: Optional[str] = None
    api_key: Optional[str] = None
    model_name: Optional[str] = None
    dimensions: Optional[int] = Field(default=None, ge=64, le=4096)
    provider_id: Optional[str] = None  # NEW: slug into configured_providers — preferred over provider when set
...
async def update_user_settings(
    ...
    if settings_update.llm_settings:
        ...
        # NEW: provider_id selects the active provider entry from configured_providers.
        # When set, it takes precedence over llm_api_type for resolution. Storing
        # both lets us roll back the multi-instance feature without losing data.
        if llm.provider_id is not None:
            user_settings.llm_provider_id = llm.provider_id if llm.provider_id else None
    ...
    if settings_update.stt_settings:
        ...
        if stt.language is not None:
            user_settings.stt_language = stt.language
    ...
    if settings_update.extraction_model_settings:
        ...
        if ext.use_main_llm_for_scene_events is not None:
            user_settings.use_main_llm_for_scene_events = ext.use_main_llm_for_scene_events
    ...
    # Save extraction api_type and provider_id from extraction_model_settings
    if settings_update.extraction_model_settings:
        ext = settings_update.extraction_model_settings
        if ext.api_type is not None:
            user_settings.extraction_model_api_type = ext.api_type if ext.api_type else None
        if ext.provider_id is not None:
            user_settings.extraction_provider_id = ext.provider_id if ext.provider_id else None
    ...
    if settings_update.embedding_model_settings:
        ...
        if emb.provider_id is not None:
            user_settings.embedding_provider_id = emb.provider_id if emb.provider_id else None
...
class TestConnectionRequest(BaseModel):
    ...
    model_name: Optional[str] = None
    provider_id: Optional[str] = None  # NEW: when set, resolves api_url/api_key/api_type from configured_providers[provider_id]
...
async def test_api_connection(
    ...
    # Resolve provider config (multi-instance aware via provider_id)
    user_settings = db.query(UserSettings).filter(
        UserSettings.user_id == current_user.id
    ).first()
    api_type, api_key, resolved_url = _resolve_request_provider(request, user_settings)
    if resolved_url:
        request.api_url = resolved_url
...
async def test_extraction_model_connection(
    ...
    # Multi-instance: if request.provider_id is set, prefer the entry's fields
    if request and getattr(request, "provider_id", None) and user_settings:
        entry = user_settings._get_provider_entry(request.provider_id)
        if entry:
            api_type = entry.get("api_type") or api_type
            api_key = api_key if api_key is not None else entry.get("api_key", "")
            url = url or entry.get("api_url", "")
