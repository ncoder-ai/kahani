...
class StoryCharacterVoiceUpdate(BaseModel):
    ...
    # Feature 3: {"presence": "low"|"normal"|"high", "expression": "internal"|"balanced"|"spoken"}
    # Only applied when explicitly present in the request (a voice-only PUT
    # must not wipe it). null clears it.
    narrative_presence: Optional[Dict[str, Any]] = None
...
class StoryCharacterResponse(BaseModel):
    ...
    narrative_presence: Optional[Dict[str, Any]] = None  # Feature 3
    ...
    for sc in story_characters:
        ...
        if character:
            result.append(StoryCharacterResponse(
                ...
                narrative_presence=sc.narrative_presence,
                name=character.name,
...
async def update_story_character_voice_style(
    ...
    # Feature 3: only touch narrative_presence if the client explicitly sent it,
    # so a voice-only PUT never clears a configured presence.
    if 'narrative_presence' in update_data.model_fields_set:
        story_character.narrative_presence = update_data.narrative_presence
    db.commit()
    ...
    return StoryCharacterResponse(
        ...
        narrative_presence=story_character.narrative_presence,
