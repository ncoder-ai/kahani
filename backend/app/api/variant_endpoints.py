...
from .story_helpers import (
    ...
    invalidate_all_for_regenerated_scene,
    llm_service,
)
    ...
    async with variant_lock:
        ...
        try:
            ...
            if flow_entry:
                ...
                # Full invalidation suite for the regenerated scene:
                # chapter summary batches, plot progress batches, scene-keyed extractions,
                # watermark rollback, and SCENE_EVENT refire. See story_helpers.invalidate_all_for_regenerated_scene.
                scene = db.query(Scene).filter(Scene.id == scene_id).first()
                if scene:
                    await invalidate_all_for_regenerated_scene(
                        scene_id=scene_id,
                        story_id=story_id,
                    ...
                    if scene.sequence_number % batch_threshold == 0:
                        # NOTE: do NOT `import asyncio` here — module-level import
                        # exists at the top of the file; shadowing it locally turns
                        # `asyncio` into a function-scope local and breaks every
                        # other asyncio.create_task call in this function with
                        # UnboundLocalError.
                    ...
                    # Schedule TTS segment extraction in background (gated
                    # inside the helper on tts_settings.use_segment_extraction).
                    try:
                        ...
                        logger.debug(f"[TTS_SEGMENT_BG] variant-create: scheduled extraction for variant {variant.id}")
                    except Exception as _e:
                        logger.warning(f"[TTS_SEGMENT_BG] variant-create: schedule failed: {_e}")
        ...
        try:
            ...
            if n_value > 1:
                ...
                if variants_data:
                    ...
                    # Full invalidation suite (chapter summary + plot progress + scene-keyed
                    # extractions + watermark rollback + SCENE_EVENT refire). The first new
                    # variant is the newly-active one (see active_variant_id below).
                    active_new_variant_id = new_variants[0].id if new_variants else None
                    if active_new_variant_id:
                        await invalidate_all_for_regenerated_scene(
                            scene_id=scene_id,
                            story_id=story_id,
                            scene_sequence=scene.sequence_number,
                            chapter_id=scene.chapter_id,
                            branch_id=scene.branch_id,
                            variant_id=active_new_variant_id,
                            user_id=current_user.id,
                            user_settings=user_settings,
                            db=db,
                        )
            ...
            if flow_entry:
                ...
                if scene_for_extraction:
                    await invalidate_all_for_regenerated_scene(
                        ...
                        chapter_id=scene_for_extraction.chapter_id,
            ...
            # Schedule TTS segment extraction in background (gated inside
            # the helper on tts_settings.use_segment_extraction). This is
            # the variant-streaming path — the most common scene/variant
            # creation route. Without this hook, freshly-generated
            # variants have no cached segments and the user pays the
            # full extraction wait on first play.
            try:
                from ..services.scene_segment_extraction_service import extract_and_cache_for_variant
                asyncio.create_task(extract_and_cache_for_variant(
                    variant_id=variant.id,
                ...
                logger.debug(f"[TTS_SEGMENT_BG] variant-streaming: scheduled extraction for variant {variant.id}")
            except Exception as _e:
                logger.warning(f"[TTS_SEGMENT_BG] variant-streaming: schedule failed: {_e}")
...
async def activate_scene_variant(
    ...
    After flipping the active variant, runs the full invalidation suite so
    downstream extractions (chronicle, lorebook, NPC, scene_events, plot
    progress, chapter summary, entity states) reflect the new active content.
    Watermarks roll back so the next scene generation's threshold sweep
    refills batch-based extractions; scene_events refires immediately.
    """
    ...
    # === Post-switch invalidation ===
    # Mirrors the regen flow: full invalidation suite + unconditional entity recalc.
    try:
        scene = db.query(Scene).filter(Scene.id == scene_id).first()
        ...
        if scene:
            ...
            # Entity states need recalc on switch (they're variant-sensitive
            # running state, not threshold-driven). force_extraction=True so
            # it runs even outside the normal batch boundary.
            asyncio.create_task(recalculate_entities_in_background(
                story_id=story_id,
                user_id=current_user.id,
                user_settings=user_settings,
            ...
            logger.info(
                f"[ACTIVATE] Post-switch invalidation complete for scene {scene_id} "
                f"variant {variant_id} (entity recalc scheduled)"
            )
    except Exception as e:
        # Don't fail the activation if invalidation has a hiccup — log + continue.
        logger.error(f"[ACTIVATE] Post-switch invalidation failed for scene {scene_id}: {e}")
        import traceback
        logger.error(f"[ACTIVATE] Traceback: {traceback.format_exc()}")
...
async def continue_scene(
    ...
    try:
        ...
        # Content changed → cached TTS segment extraction is stale.
        current_variant.tts_segments = None
        _bg_variant_id_for_tts_reextract = current_variant.id
        db.commit()
        # Re-extract in background if user opted in (gated inside the helper).
        try:
            from ..services.scene_segment_extraction_service import extract_and_cache_for_variant
            asyncio.create_task(extract_and_cache_for_variant(
                variant_id=_bg_variant_id_for_tts_reextract,
                user_id=current_user.id,
                user_settings=user_settings or {},
            ))
        except Exception as _e:
            logger.warning(f"[TTS_SEGMENT_BG] continue: schedule failed: {_e}")

        # Full invalidation suite for the continued/regenerated scene
        await invalidate_all_for_regenerated_scene(
            scene_id=scene_id,
            story_id=story_id,
            scene_sequence=scene.sequence_number,
    ...
    async def generate_continuation_stream():
        try:
            ...
            try:
                ...
                logger.warning(f"[TTS_SEGMENT_BG] continue-stream: schedule failed: {_e}")
...
async def update_scene_variant(
    ...
    async with edit_lock:
        ...
        try:
            ...
            # Content changed → cached TTS segment extraction is stale.
            # Drop it; the next play will re-extract if the user has the
            # segment-extraction toggle on (otherwise this is a no-op).
            variant.tts_segments = None
            ...
            db.commit()
            db.refresh(variant)
            # Re-extract TTS segments in background (gated inside the helper).
            try:
                from ..services.scene_segment_extraction_service import extract_and_cache_for_variant
                asyncio.create_task(extract_and_cache_for_variant(
                    variant_id=variant.id,
                ...
                logger.warning(f"[TTS_SEGMENT_BG] edit: schedule failed: {_e}")
            ...
            # NOTE: do NOT do `import asyncio` here — there's a module-level
            # import already, and a function-local import would shadow it
            # making earlier `asyncio.create_task` calls in this same
            # function raise UnboundLocalError.
