...
# ============================================================================
# TTS CHARACTER VOICES (per-story, per-provider mapping)
# ============================================================================

class TTSCharacterVoicesUpdate(BaseModel):
    """Body for PUT /api/stories/{id}/tts-character-voices.

    Map of lowercased character name → provider voice id. Use the
    special key 'narrator' for narration. Pass an empty object to
    clear all mappings for the given provider.
    """
    voices: Dict[str, str]


def _validate_provider(provider: str) -> str:
    """Light validation: ensure caller provided a non-empty string.

    We accept any provider name (registry membership is enforced at
    synthesis time) so that map entries can be saved before the user
    completes provider setup.
    """
...
@router.get("/{story_id}/tts-character-voices")
async def get_tts_character_voices(
    story_id: int,
    provider: str,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """Return the saved character→voice map for this story + provider.

    Returns an empty `{voices: {}}` when nothing has been configured
    yet. Lookups are case-insensitive — keys are stored lowercased.
    """
    ...
    full_map = story.tts_character_voices or {}
    return {"voices": full_map.get(provider, {})}


@router.put("/{story_id}/tts-character-voices")
async def put_tts_character_voices(
    story_id: int,
    body: TTSCharacterVoicesUpdate,
    provider: str,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """Replace the character→voice map for this story + provider.

    Other providers' maps inside `tts_character_voices` are preserved
    untouched. Empty/whitespace voice values cause the entry to be
    removed (cleanest sentinel for "fall back to default voice").
    """
    provider = _validate_provider(provider)
    ...
    # Normalise: lowercase keys, drop empty values.
    cleaned: Dict[str, str] = {}
    for raw_name, raw_voice in (body.voices or {}).items():
        if not raw_name or not isinstance(raw_name, str):
            continue
        if not raw_voice or not isinstance(raw_voice, str) or not raw_voice.strip():
            continue
        cleaned[raw_name.strip().lower()] = raw_voice.strip()

    full_map = dict(story.tts_character_voices or {})
    if cleaned:
        full_map[provider] = cleaned
    else:
        full_map.pop(provider, None)
    story.tts_character_voices = full_map
    # SQLAlchemy needs a hint that the JSON field has mutated.
    from sqlalchemy.orm.attributes import flag_modified
    flag_modified(story, "tts_character_voices")
    db.commit()
