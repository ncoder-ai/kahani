...
async def invalidate_extractions_for_scene(
    ...
    1. Deletes all extractions for the scene (CharacterMemory, PlotEvent, NPCMention, SceneEmbedding)
       and rolls back chapter watermarks (last_extraction_scene_count,
       last_chronicle_scene_count) so the next threshold sweep re-extracts.
    ...
    try:
        ...
        # Invalidate extractions for the scene (also rolls back chapter watermarks)
...
async def invalidate_all_for_regenerated_scene(
    scene_id: int,
    story_id: int,
    scene_sequence: int,
    ...
    """
    Full invalidation suite for a scene whose active variant just changed.
    Use after variant regen (POST /variants, /continue) or variant switch (/activate).

    This consolidates the four invalidation calls that used to be copy-pasted
    across 5 sites in variant_endpoints.py, plus the missing plot_progress
    invalidate and the SCENE_EVENT refire:

      1. Chapter summary batches → recomputes last_summary_scene_count
      2. Plot progress batches → recomputes last_plot_extraction_scene_count
      3. Scene-keyed extractions (chronicle/lorebook/NPC/scene_events/etc) →
         wipes rows and rolls back last_extraction_scene_count + last_chronicle_scene_count
      4. Entity state batches → invalidated for caller to recalc
      5. SCENE_EVENT refire scheduled (no watermark on scene_events → needs explicit refire)

    Entity recalc and TTS extraction are NOT scheduled here — caller owns those
    decisions (regen endpoints do entity recalc conditionally at threshold;
    activate does it unconditionally).
    """
    import asyncio

    # 1. Chapter summary batches (recomputes last_summary_scene_count from remaining batches)
    from .chapters import invalidate_chapter_batches_for_scene
    try:
        invalidate_chapter_batches_for_scene(scene_id, db)
    except Exception as e:
        logger.error(f"[INVALIDATE_ALL] chapter_summary invalidate failed for scene {scene_id}: {e}")

    # 2. Plot progress batches (recomputes last_plot_extraction_scene_count)
    try:
        from ..services.chapter_progress_service import ChapterProgressService
        ChapterProgressService(db).invalidate_plot_progress_batches_for_scene(scene_id)
    except Exception as e:
        logger.error(f"[INVALIDATE_ALL] plot_progress invalidate failed for scene {scene_id}: {e}")

    # 3. Scene-keyed extractions + watermark rollback + entity batches
    ...
    # 4. SCENE_EVENT refire (per-scene cadence — no watermark to roll back)
    try:
        from .story_tasks.background_tasks import run_scene_event_extraction_for_scene_in_background
        from .scene_endpoints import _defer_until_tts_segment_done
        asyncio.create_task(_defer_until_tts_segment_done(
            ...
            run_scene_event_extraction_for_scene_in_background(
                ...
                scene_generation_context=scene_generation_context or {},
            ),
        ))
        logger.info(f"[INVALIDATE_ALL] Scheduled SCENE_EVENT refire for scene {scene_id} variant {variant_id}")
    except Exception as e:
        logger.error(f"[INVALIDATE_ALL] SCENE_EVENT refire scheduling failed for scene {scene_id}: {e}")
