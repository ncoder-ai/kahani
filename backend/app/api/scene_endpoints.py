...
from .story_tasks import (
    ...
    run_scene_event_extraction_for_scene_in_background,
...
async def _defer_until_tts_segment_done(variant_id: int, coro):
    """Wait for any in-flight TTS segment extraction for ``variant_id`` to
    finish before running ``coro``. Used to keep secondary extractions
    (chapter summary, inline contradiction check, character moments, plot
    events, chronicle, working memory) off the extraction-LLM queue while
    TTS is preparing the per-speaker segments cache for the user's first
    play tap.

    Without this gate, all post-scene background tasks asyncio.create_task
    at roughly the same time and queue serially on the single kobold
    instance — a slow non-TTS extraction (e.g. working memory) can push
    TTS prep from ~5s to ~45s, which the user feels as a long silent gap
    on the play button. ``wait_for_in_flight`` returns immediately if no
    TTS extraction is registered for this variant.
    """
    import asyncio as _asyncio
    try:
        from ..services.scene_segment_extraction_service import wait_for_in_flight
        # Generous timeout — TTS polish on Ministral-8B can occasionally hit
        # 70s+ on degenerate prompts; we'd rather block other extractions
        # than fire them concurrently and re-create the queue jam.
        await wait_for_in_flight(variant_id, timeout=90.0)
    except Exception as e:
        logger.warning(f"[DEFER_AFTER_TTS] wait_for_in_flight failed for variant {variant_id}: {e}; running deferred task anyway")
    return await coro


def _resolve_scene_focus(focus_character: str, context: Dict[str, Any]) -> Optional[str]:
    """Validate a per-scene focus-character request against the active cast.

    Spotlight is ephemeral (per scene) and additive: an unknown/empty value
    resolves to None so the prompt is byte-identical to the no-focus case.
    Returns the canonical character name (matched case-insensitively against
    the chapter's active characters) or None.
    """
    if not focus_character or not focus_character.strip():
        return None

    requested = focus_character.strip().lower()
    characters = context.get("characters") if isinstance(context, dict) else None
    if isinstance(characters, dict):
        candidates = characters.get("active_characters", []) or []
    ...
    for char in candidates:
        name = (char.get("name") or "").strip() if isinstance(char, dict) else ""
        if name and name.lower() == requested:
            return name

    logger.info(f"[SCENE FOCUS] Requested focus '{focus_character}' not in active cast — ignoring")
    return None
...
async def generate_scene(
    ...
    if content_mode == "user_scene":
        ...
        context["scene_focus"] = _resolve_scene_focus(focus_character, context)
    ...
    # WORKING MEMORY extraction intentionally NOT scheduled here.
    # Audited 2026-05-15: working_memory is write-only — the extracted
    # recent_focus/character_spotlight/pending_items are read into
    # context_manager._build_story_focus() but story_focus never reaches
    # the LLM prompt builder (service.py), so the per-scene extraction LLM
    # call had zero downstream effect. update_working_memory_in_background
    # is kept intact for re-enable if working_memory is ever wired into
...
async def generate_scene_streaming_endpoint(
    ...
    # Per-scene spotlight (ephemeral): validated against the active cast,
    # injected into the post-cache-break task message only. Inherited by the
    # multi-gen path's scene_context = context.copy() below.
    context["scene_focus"] = _resolve_scene_focus(focus_character, context)
    ...
    async def _run_generation_task(state):
        ...
        try:
            ...
            # === TTS SEGMENT EXTRACTION (fired FIRST so autoplay/play has cache) ===
            # Multi-speaker TTS needs per-speaker segments before it can dispatch
            # to vibevoice. Doing this lazily on the first play call adds a 5-15s
            # silent gap on the user's tap; doing it eagerly here moves that work
            # into the post-generation window when the user is reading the new
            # scene's text. The lazy path uses `wait_for_in_flight` to dedupe,
            # so this fires-and-forgets safely — a play tap during this window
            # joins the in-flight task instead of starting a duplicate.
            try:
                from ..services.scene_segment_extraction_service import extract_and_cache_for_variant
                asyncio.create_task(extract_and_cache_for_variant(
                    variant_id=variant.id,
                    user_id=current_user.id,
                    user_settings=user_settings or {},
                ))
                logger.debug(f"[TTS_SEGMENT_BG] Scheduled extraction for variant {variant.id} (priority: ahead of entity/plot/embedding)")
            except Exception as e:
                logger.error(f"[TTS_SEGMENT_BG] Failed to schedule extraction: {e}")
            ...
            if active_chapter:
                try:
                    ...
                    if scenes_since_summary >= summary_threshold:
                        ...
                        # Deferred until TTS segment extraction releases the LLM queue —
                        # see _defer_until_tts_segment_done.
                        ...
                        if scenes_since_extraction >= extraction_threshold:
                            ...
                            asyncio.create_task(_defer_until_tts_segment_done(
                                ...
                                run_extractions_in_background(
                                    ...
                                    skip_entity_states=enable_inline_check,
                                ),
                            ))
            ...
            # === SCENE EVENT EXTRACTION (recall index — per scene, every scene) ===
            # Independent of the 5-scene character_extraction_threshold so the recall
            # index stays fresh on every scene. Honours use_main_llm_for_scene_events
            # and use_cache_friendly_prompts user settings.
            try:
                logger.info(f"[SCENE_EVENT] Scheduling per-scene event extraction for scene {scene.sequence_number}")
                asyncio.create_task(_defer_until_tts_segment_done(
                    variant.id,
                    run_scene_event_extraction_for_scene_in_background(
                        story_id=story_id,
                ...
                logger.error(f"[SCENE_EVENT] Failed to schedule per-scene extraction: {e}")
                import traceback
                logger.error(f"[SCENE_EVENT] Traceback: {traceback.format_exc()}")
            ...
            # === WORKING MEMORY UPDATE — intentionally disabled ===
            # Audited 2026-05-15: working_memory is write-only dead code.
            # The extraction populates recent_focus/character_spotlight/
            # pending_items, which context_manager._build_story_focus() reads
            # into base_context["story_focus"] — but story_focus is never
            # formatted into any prompt section and appears zero times in
            # service.py (the LLM prompt builder). So this per-scene extraction
            # LLM call had no downstream effect. The function
            # update_working_memory_in_background is kept intact; only the
            # scheduling is removed. Re-enable here if working_memory is ever
            # wired into the cache-friendly prefix.

            # Relationship extraction removed — low ROI, separate LLM call not justified

            # TTS segment extraction is now scheduled at the TOP of PRIORITY 3
            # (above auto-summary/entity/plot) so autoplay hits the cache faster.
            # See comment block near `extract_and_cache_for_variant` above.
