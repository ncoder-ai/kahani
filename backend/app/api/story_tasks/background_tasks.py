    ...
    try:
        ...
        for batch_start in range(0, total_scenes, batch_size):
            ...
            try:
                ...
                # Prompts now live in prompts.yml (interaction_extraction)
                from ...services.llm.prompts import prompt_manager
                batch_prompt = prompt_manager.get_prompt(
                    "interaction_extraction", "user",
                    interaction_types_json=json.dumps(interaction_types),
                    character_names_csv=", ".join(character_names),
                    already_found_text=already_found_text,
                    batch_content="".join(batch_content_parts),
                )
                system_prompt = prompt_manager.get_prompt("interaction_extraction", "system") \
                    or "You are a precise story analyst. Only extract interactions you are certain about. When in doubt, exclude. Return valid JSON only."
...
async def run_scene_event_extraction_background(
    ...
    # Honour use_main_llm_for_scene_events — when True, force main LLM even if
    # the extraction LLM is enabled (mirrors per-scene live extraction).
    extraction_settings = user_settings.get('extraction_model_settings', {})
    force_main_llm = extraction_settings.get('use_main_llm_for_scene_events', False)

    extraction_service = None
    if not force_main_llm and extraction_settings.get('enabled', False):
        try:
            extraction_service = ExtractionLLMService.from_settings(user_settings)
            if extraction_service:
    ...
    main_llm = UnifiedLLMService() if not extraction_service else None
    if force_main_llm:
        logger.info(f"[EVENT_EXTRACT] Forcing main LLM (use_main_llm_for_scene_events=True)")
    ...
    try:
        ...
        for scene in scenes:
            try:
                ...
                # Use _store_scene_events so events get embedded (cosine-search-ready).
                # Inline-inserting SceneEvent rows here would skip the embedding step
                # and the recall agent's pgvector search would silently return [] for
                # every query (filter is `embedding IS NOT NULL`).
                from ...services.semantic_integration import _store_scene_events
                world_id = scene.story.world_id if scene.story else None
                stored = await _store_scene_events(
                    db=extraction_db,
                    events=scene_events_data,
                    story_id=story_id,
                    scene_id=scene.id,
                    scene_sequence=scene.sequence_number,
...
async def run_scene_event_extraction_for_scene_in_background(
    story_id: int,
    scene_id: int,
    scene_sequence: int,
    chapter_id: Optional[int],
    ...
    """Per-scene event extraction for the recall index.

    Fires after every scene generation (similar to plot extraction). Uses
    the cache-friendly single-scene path so it respects both:
      - use_main_llm_for_scene_events (force main LLM over extraction LLM)
      - use_cache_friendly_prompts (cache-friendly multi-message vs simple)

    Replaces (and supersedes) the dead extraction path inside
    batch_process_scene_extractions, where scene_events writes were gated
    on len(scenes_data) == 1 and therefore never fired with the default
    character_extraction_threshold of 5.
    """
    try:
        from ...services.llm.service import UnifiedLLMService
        from ...services.llm.extraction_service import extract_json_robust
        from ...services.semantic_integration import _store_scene_events
        from ...models import Story

        # Short delay so the generating session has committed.
        await asyncio.sleep(0.3)

        extraction_db = SessionLocal()
        try:
            scene = extraction_db.query(Scene).filter(Scene.id == scene_id).first()
            if not scene:
                logger.warning(f"[SCENE_EVENT] Scene {scene_id} not found, skipping")
                return
            ...
            if not flow or not flow.scene_variant:
                logger.warning(f"[SCENE_EVENT] No active variant for scene {scene_id}, skipping")
                return
            scene_content = flow.scene_variant.content
            ...
            story_obj = extraction_db.query(Story).filter(Story.id == story_id).first()
            world_id = story_obj.world_id if story_obj else None

            ext_settings = user_settings.get('extraction_model_settings', {}) if user_settings else {}
            force_main_llm = ext_settings.get('use_main_llm_for_scene_events', False)

            llm_service = UnifiedLLMService()

            try:
                response = await llm_service.extract_scene_events_cache_friendly(
                    scene_content=scene_content,
                    character_names=character_names,
                    context=scene_generation_context or {},
                    user_id=user_id,
                    user_settings=user_settings,
                    db=extraction_db,
                ...
                logger.error(f"[SCENE_EVENT] LLM call failed for scene {scene_sequence}: {e}")
                return

            if not response:
                logger.warning(f"[SCENE_EVENT] Empty response for scene {scene_sequence}")
                return

            result = extract_json_robust(response)
            events = result.get('events', []) if isinstance(result, dict) else []
            if not events:
                logger.info(f"[SCENE_EVENT] No events extracted for scene {scene_sequence}")
                return
            ...
            extraction_db.commit()
            logger.info(
                f"[SCENE_EVENT] Stored {stored} events for scene {scene_sequence} "
                f"(force_main_llm={force_main_llm})"
            )
        finally:
            extraction_db.close()
    except Exception as e:
        logger.error(f"[SCENE_EVENT] Background task failed: {e}")
        import traceback
